// start_cpp/hf_rpe/src/hf_rpe_parser.cpp
//
// HF Receiver/Processor/Exciter parser DLL  —  1 MHz to 30 MHz
// Protocol: DP-ECM-1071-6333-V1-ICD-0V06
//
...
//
// Every other group/unit falls through to a safe raw_hex envelope so the
// bridge never crashes on an unrecognised message.
#include "sdfc_abi.h"
...
static constexpr const char* HW_NAME = "hf_rpe";   // 1 – 30 MHz

// =============================================================================
// Per-message decoders
// Each receives: payload pointer, payload byte count, JsonWriter to append into.
// =============================================================================

// ─── Group 100 / Unit 2  —  Get System Version Details ───────────────────────
// 20 bytes: fw_ver(u32) drv_ver(u32) fpga_ver(u32) bsp_ver(u32)
//           processor_id(u16) fpga_type_id(u16)
...
// 16 bytes: internal_temp(f32) external_temp(f32) cpu_temp(f32) fpga_temp(f32)
...
// ─── Group 101 / Unit 40  —  FH (Frequency Hopping) Detection Report ─────────
// Header: hopper_count(u16) reserved(u16)
// S_HOPPER_DATA × hopper_count  (40 bytes each):
//   hopper_number(u32) @0
//   min_freq_mhz(f32)  @4   → output Hz
//   max_freq_mhz(f32)  @8   → output Hz
//   pulse_len_ms(f32)  @12  → output s
//   inter_hop_ms(f32)  @16  → output s
//   detected_count(u32)@20
//   toa_h(u8) toa_m(u8) toa_s(u8) reserved(u8) @24
//   power_dbm(f32)     @28
//   active(u16)        @32
//   reserved(u16)      @34
//   snr_db(f32)        @36
...
// Header (16 bytes):
//   start_freq_mhz(u32) @0  stop_freq_mhz(u32) @4
//   step_khz(u32) @8        point_count(u32) @12
// Payload: power_dbm[point_count](f32) @16
// Frequencies output in Hz; power in dBm.
// Points capped at 2048 in the JSON to keep messages manageable.
static void decode_wideband_fft(const uint8_t* p, int n, JsonWriter& w) {
    ...
    // Guard against truncated payload.
...
// ─── Group 101 / Unit 70  —  FF (Fixed Frequency) Detection Report ───────────
// Header: freq_count(u16) reserved(u16)
// S_FF_DATA × freq_count  (32 bytes each):
//   freq_mhz(f32)     @0   → output Hz
//   power_dbm(f32)    @4
//   bandwidth_khz(f32)@8   → output Hz
//   toa_h(u8) toa_m(u8) toa_s(u8) reserved(u8) @12
//   duration_ms(f32)  @16  → output s
//   active(u16)       @20
//   reserved(u16)     @22
//   snr_db(f32)       @24
//   reserved(f32)     @28
...
// Header: burst_count(u16) reserved(u16)
// S_BURST_DATA × burst_count  (36 bytes each):
//   center_freq_mhz(f32) @0  → output Hz
//   power_dbm(f32)       @4
//   bandwidth_khz(f32)   @8  → output Hz
//   toa_h(u8) toa_m(u8) toa_s(u8) reserved(u8) @12
//   duration_ms(f32)     @16 → output s
//   rep_period_ms(f32)   @20 → output s
//   burst_count_det(u32) @24
//   active(u16)          @28
//   reserved(u16)        @30
//   snr_db(f32)          @32
...
// =============================================================================
// ABI: extract_frame
// Delegates to the shared SDFC frame scanner (sdfc_frame.cpp).
// Returns: 1=command, 2=response, 0=incomplete, -1=corrupt
// =============================================================================
...
extern "C" SDFC_EXPORT const char* parse_message(const uint8_t* frame, int frame_len,
    ...
    if (frame_type == FRAME_RESPONSE) {
        ...
        // TODO: 106/* jamming, 109/12 date-time, 111/* Group11 ops
    }
...
// =============================================================================
// ABI: format_response
// Encodes JSON {group_id, unit_id, status [, payload_hex]} into a binary
// DRS->SDFC response frame that can be sent over TCP to the hardware.
// =============================================================================
