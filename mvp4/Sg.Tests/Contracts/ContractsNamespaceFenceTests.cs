...
public class ContractsNamespaceFenceTests
{
    ...
    public void Contracts_namespace_must_not_import_AGI_types()
    {
        ...
        while (dir is not null && !File.Exists(Path.Combine(dir.FullName, "Sg.sln")))
            dir = dir.Parent;
        dir.Should().NotBeNull("solution root should be found by walking up from test bin folder");
        var contractsDir = Path.Combine(dir!.FullName, "Sg.Domain", "Contracts");
        ...
        violations.Should().BeEmpty(
            $"Sg.Domain.Contracts.* must not reference AGI.* (found: {string.Join(", ", violations.Select(v => $"{v.file}:{v.idx + 1}"))})");
    }
}
