...
- Unit (Vitest + React Testing Library for the React webapp)
...
4. **Add the items called out in §6.1 + §6.2 as new backlog rows.** **Reconciled 2026-05-31:** the IDs originally proposed here (B1.17–B1.29) had collided with later backlog reuse (B1.17 became auth flows, B1.18 vendoring, B1.19 scenario lifecycle, B1.20 NTP), so these items were analysed here but never tracked. They are now registered under fresh IDs **B1.21–B1.42** with owners + targets in [design-backlog.md §1 "Reconciled pre-mortem items"](design-backlog.md). The originally-listed set — IRS store, SRS+ATP+STD+STR, Sg.App UI, security, STK install diagnostic, user manuals, maintenance, retention, licence expiry, IRS revision, power recovery, licence catalogue, customer issue reporting — all carry forward into that range.
