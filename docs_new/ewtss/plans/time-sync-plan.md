...
**Tech Stack:** Meinberg NTP (Windows MSI vendored); Python 3.12 + FastAPI + asyncio + aiokafka (drs-server, drs-bridge); Pydantic for YAML profile validation; C# .NET 8 WPF + System.Net.Http + System.Text.Json (Sg.App); React + Vite + JavaScript (JSX) per ADR-018 (DRS webapp — the B1.2 framework decision picked React + JavaScript on 2026-06-25; the Phase 6 tasks below were authored in Angular/TypeScript syntax but were implemented in React + JavaScript — `useTimeSyncStatus` query hook, `TimeSyncCard`, `TimeSyncRow`).
