...
> **⚠️ Superseded detail — Docker was dropped.** This plan was executed (2026-05-21)
> against a `docker-compose`-based local broker, and the steps below still describe that
> approach. The `infrastructure/docker-compose.yml` was **subsequently removed**: Docker is
> never used in the air-gapped Windows deployment, so the dev path now runs the Kafka KRaft
> broker **natively** to match production. This document is kept as the historical
> implementation record — for current setup, follow
> [`infrastructure/kafka/README.md`](../../../infrastructure/kafka/README.md) and the
> [Kafka infra design spec](../specs/kafka-infra-layer-design.md), not the `docker compose`
> commands here.
