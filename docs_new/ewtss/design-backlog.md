...
- Exercise control bar — **Scenario mode** (Standalone / Integrated toggle; playback start / pause / resume / stop / replay). *(Random-mode config + run-control is a DRS-side surface — see below — per [ADR-020](decision-record.md#adr-020--scenario-is-the-definition-exercise-is-the-run-drs-server-owned-random-mode-is-ws2-only).)*
...
- **Random-mode parameter-range editor (per variant) + run-control** (start / stop; Standalone / Integrated) — WS2-only, no SG workstation ([ADR-020](decision-record.md#adr-020--scenario-is-the-definition-exercise-is-the-run-drs-server-owned-random-mode-is-ws2-only); flow specced in [random-mode-execution-design](specs/random-mode-execution-design.md))
...
### 🎯 B1.2 — DRS webapp framework decision (decided: React)

**RFQ reference:** Annexure A.1 §1.4, §1.5, §2.5 — implied by the DRS application's user-facing surface requirements.
**Decision (2026-06-25): React in JavaScript** (Vite), per [ADR-018](decision-record.md#adr-018--ws2-drs-webapp-required-browser-frontend-on-the-drs-workstation-served-by-drs-server). Primary driver: the org's React teams standardise on **JavaScript**, and Angular mandates TypeScript; plus an Angular-developer shortage and org-wide React momentum. Angular was the original preference (skill alignment with E + Mode B reuse). To keep one v2 web-frontend stack, the Mode B `Sg.Web` SPA is re-pointed to React + JavaScript too ([ADR-009](decision-record.md#adr-009--browser-spa-framework-mode-b-react--cesiumjs)).
**Owner:** project lead + architecture lead, with G's input on skill alignment.
**Blocking dependencies:** none — decided.
...
**Scope:** protocol selection (NTP / PTP / custom); tolerance specification (max acceptable skew per consumer); operator-facing time-sync admin UI (admin view in Sg.App showing per-subsystem skew); time-source distribution to drs-bridge, drs-server, DRS webapp, and Entity Controllers via the integration API; **WS2 local-clock fallback so DRS-Engineer, WS2-only Random exercises remain runnable with WS1 (the Stratum-1 source) powered off** (added per [ADR-020](decision-record.md#adr-020--scenario-is-the-definition-exercise-is-the-run-drs-server-owned-random-mode-is-ws2-only); designed in [time-sync-design §4.5 + §7.1](specs/time-sync-design.md)).
...
**Scope:** split by persona per [ADR-020](decision-record.md#adr-020--scenario-is-the-definition-exercise-is-the-run-drs-server-owned-random-mode-is-ws2-only):
- **SG side (Sg.App, WS1):** Scenario-mode computed-exercise picker; Integrated-mode readiness panel (entity-controller reachability check); new-emitter-mid-planning flow (RFQ §B).
- **DRS side (drs_webapp, WS2 — owner G + drs-server):** Random-mode parameter-range editor per variant + run-control (start/stop, Standalone/Integrated). WS2-only, no SG workstation required. The backend flow (drs-server endpoints, `exercises` write, `drs.session.control` publishing) is specced in [random-mode-execution-design](specs/random-mode-execution-design.md); only the visual UI layout remains a B1.1 wireframe item.
**Owner:** B (SG-side surfaces) + G (DRS-side Random surface) + E/F (drs-server endpoints) + A (drs-bridge coordination).
**Target:** Wireframe in B1.1; Random-mode execution flow specced (see spec above); implementation across weeks 6–11 per execution plan.
**Status:** Open — Random-mode execution *flow* design done ([spec](specs/random-mode-execution-design.md)); UI layout + Scenario-side picker still open.
...
**Decision (resolved — design in [security-design §5](specs/security-design.md)):**
- **Catalogue:** three roles — **Admin, SG Operator, DRS Engineer** (no `observer`). Schema enum `admin | sg_operator | drs_engineer`.
- **Multi-role:** a user may hold multiple roles (effective permissions = union) — a `user_roles` join table replaces the single `users.role_id` FK ([developer-handbook §10.3](developer-handbook.md#10-database-schema-reference)).
- **Admin is least-privilege:** management + view only, not authoring/run; operational authority comes from also holding SG Operator / DRS Engineer.
- **Feature matrix** (10 features incl. `exercise.run.scenario` → SG Operator and `exercise.run.random` → DRS Engineer, per [ADR-020](decision-record.md#adr-020--scenario-is-the-definition-exercise-is-the-run-drs-server-owned-random-mode-is-ws2-only)) defined in security-design §5.
- **Shared RBAC store** on WS2 PostgreSQL (unchanged).
**Owner:** E + architecture lead.
**Target:** Role catalogue by week 3 of v2 hardening; implementation week 4–8.
**Status:** **Design done** (catalogue + feature matrix + schema shape). Implementation open — `user_roles` migration, feature seed data, per-endpoint permission checks (B1.28 RBAC audit).
...
**Target:** Designed by week 4 of v2 hardening; implementation across weeks 5–9 (alongside the base RBAC + JWT work in [E's allocation §3.6](v2-execution-plan.md#36-e-react-cross-trained--drs-server-rest--auth--reports)). Each flow gets a Mermaid sequence diagram added to [command-flows.md §1](command-flows.md#1-authentication).
...
**Status:** **Process documented (2026-06-05); webapp npm deps recorded (2026-06-25).** [Developer Handbook §15.8](developer-handbook.md) defines the `packages/` layout, the `pip download` → offline-verify → PR-time-check procedure, and the runtime-installer pattern; `packages/wheels/` + `packages/installers/` + `packages/npm/` scaffolded with READMEs; the B1.43 deps (six roster-store wheels + Postgres/Kafka/JVM) **and the drs-webapp npm deps (React/Vite/test/lint, all MIT)** are recorded in [`packages/THIRD-PARTY-LICENCES.md`](../../packages/THIRD-PARTY-LICENCES.md). At **runtime** the webapp ships as a **pre-built static bundle** (`dist/drs-webapp/`) served by drs-server, so WS1/WS2 run no Node/npm. But the programme requires **build-from-source on the secure segment** (decision 2026-06-25), so the air-gapped build host must carry the full offline build chain: the vendored **Node.js 22 toolchain** ([`packages/installers/nodejs/`](../../packages/installers/nodejs/README.md)) + the **offline npm cache** ([`packages/npm/README.md`](../../packages/npm/README.md)), running `npm ci --offline && npm run build` with no internet. `package-lock.json` SRI-pins the full transitive set; the cache must be populated on a Windows-x64 host (esbuild/Rollup ship platform-specific prebuilt binaries). **Remaining = the binary vendoring itself** (wheels + Node archive + npm cache + installers), deliberately deferred to the pre-delivery freeze against the frozen dep set (per the timing rule in §15.8; ad-hoc still fine meanwhile). Node's bundled **npm CLI is Artistic-2.0** → minor §15.7 confirmation; JVM choice gated on [B1.44](#-b144--jvm-licence-for-the-kafka-runtime-gplv2--classpath-exception).
...
| B1.30 | **drs-webapp e2e + coverage gate** (Playwright smoke + Vitest coverage threshold) | 🟡 | PM §3.4 / strategy §6.1 | G | With B1.2 framework decision (React). Open. |
...
**Status:** **Closed (design); implementation in progress.**
- **Plan 1 (contracts + drs-bridge)** — **merged to `main`** (PR #1): drs.roster contract + compacted topic, per-instance servers, roster consumer, hot-reload, framing probation, per-instance health. Full suite green locally (48/0 after a local cmake install built the reference parser).
- **Plan 2a (DB foundation + roster store + CRUD)** — **implemented** (PR #2, branch `feat/b1-43-roster-store`, [`plans/drs-roster-store-plan.md`](plans/drs-roster-store-plan.md)): SQLAlchemy 2.0 async + Alembic foundation, `roster`/`roster_entry`/`roster_revision` tables, pure §5.4 validation, `RosterRepository`, read/write/export-import endpoints, `/roster/*` OpenAPI baselined. **Fully verified locally against real PostgreSQL + Kafka (drs-server 52/0, drs-bridge 48/0, zero skips)** — the live run caught + fixed a tz-aware/naive model↔migration mismatch and two Kafka-test defects.
- **Plan 2b (publisher + presence + readiness + allocator)** — **implemented** (branch `feat/b1-43-roster-reconcile`, [`plans/drs-roster-reconcile-plan.md`](plans/drs-roster-reconcile-plan.md)): `RosterPublisher` → compacted `drs.roster` (key `active`) on every write + at startup; drs-bridge `instance.disconnected`; drs-server `PresenceConsumer` + in-memory presence; `POST /exercise/readiness` (`configured`/`reachable`/`time_synced` + `roster_drift` + `ready`); port-allocation engine for `allocated` entries; `/exercise/readiness` OpenAPI baselined. Verified locally against real PostgreSQL + Kafka (drs-server 65/0, drs-bridge 49/0).
- **Plan 3** (Sg.App readiness panel + DRS-webapp IP/network-config editing surface, after [B1.1](#-b11--detailed-ux-wireframes-sg-operator-and-drs-engineer-surfaces) wireframes) — the only remaining B1.43 work.

### 🟠 B1.44 — JVM licence for the Kafka runtime (GPLv2 + Classpath Exception)

**Relates to:** [B1.39](#reconciled-pre-mortem-items-b121b142) (third-party licence catalogue + allow-list). Surfaced 2026-06-05 during B1.43 Plan 2a infra verification.

**Issue:** v2's telemetry bus is Kafka, which requires a **JVM** at runtime. OpenJDK / Eclipse Temurin is **GPLv2 with the Classpath Exception**. The CPE is specifically designed so that running/redistributing the JVM does not pull copyleft into the applications that run on it — but the [Developer Handbook §15.7](developer-handbook.md#157-third-party-licence-allow-list) block-list lists "GPL (all versions)", so adopting a JVM for the air-gapped delivery needs an explicit **project-lead decision**: accept GPLv2+CPE (record the rationale), or select a JVM distribution / Kafka alternative with terms the project prefers. Recorded in [`packages/THIRD-PARTY-LICENCES.md`](../../packages/THIRD-PARTY-LICENCES.md).

**Owner:** project lead + D. **Sev:** 🟠 (blocks air-gap release packaging of the Kafka runtime). **Target:** decide before the first air-gap packaging milestone. **Status:** Open.

### 🟡 B1.45 — DRS webapp `checkJs` type-safety gate (DTO-boundary drift mitigation)

**Relates to:** [ADR-018](decision-record.md#adr-018--ws2-drs-webapp-required-browser-frontend-on-the-drs-workstation-served-by-drs-server) dropped-TypeScript trade-off; [Risk Register D2](risk-register.md) (C# DTO ↔ JS mirror drift). Raised 2026-06-25 during the Angular → React + JavaScript migration.

**Issue:** the webapp is plain JavaScript by decision (org React teams standardise on JS). The one cost is the loss of compile-time checking on the C# `Sg.Domain.Contracts` DTO boundary the webapp consumes over REST/WS — the migration's headline risk. The mitigation is half-built: JSDoc `@typedef`s document the DTO shapes (`SyncStatus`, `TimeSyncStatus` in [`drs-webapp/src/services/timeSync.js`](../../drs-webapp/src/services/timeSync.js)) and `jsconfig.json` ships with `checkJs: false`, so today they are **documentation/editor hints only — not enforced**. Turning them into an enforced check (`checkJs`) is **not a free flag**: a useful gate re-introduces, as **dev/CI-only** deps (never shipped; nobody writes TypeScript), `typescript` (to run `tsc --noEmit` in CI) and `@types/react` + `@types/react-dom` (so React/JSX aren't typed `any`). That partially reverses the migration's "remove TypeScript dependencies" directive, so it is a conscious project-lead call, not a silent default.

**Options:**
- **A — editor-only:** flip `jsconfig.json` → `checkJs: true`. VS Code surfaces JSDoc type errors with zero added deps; nothing enforces them in CI.
- **B — enforced gate:** A + re-add `typescript` / `@types/react*` (dev/CI only) + a `typecheck` script (`tsc -p jsconfig.json --noEmit`) + a CI step.
- **C — status quo:** keep `checkJs: false`; JSDoc stays documentation; rely on the C#-side JSON round-trip tests + OpenAPI as the contract guard.

**Why decide soon (not vague-"later"):** it is cheapest to establish the discipline while the webapp is a single feature slice; deferring lets unchecked JS accumulate. Best decided before the per-variant Phase-6 surfaces (monitor-scan, log viewer, IP config) land.

**Owner:** G + project lead (the "remove TS" directive is the project lead's to relax). **Sev:** 🟡 (quality/safety; **not** a release blocker — the contract is also guarded by JSON round-trip tests + OpenAPI). **Target:** decide before the Phase-6 surface build-out (weeks 9–11). **Status:** Open — deferred pending the dev-dependency-vs-safety tradeoff call.
...
**Scope:** React + CesiumJS SPA wireframes; OpenAPI freeze; TypeScript view-model mirroring strategy.
