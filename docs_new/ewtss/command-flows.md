...
sequenceDiagram
    ...
    drs-server-->>Sg.App: JWT (signed, contains the user's roles + effective features = union across roles)
    Sg.App->>Sg.App: Cache JWT in-memory, bind to RBAC view-gating
    Sg.App-->>Op: Main window opens, gated by the union of the user's roles
```

**Notes:**
- RBAC view-gating ensures the operator sees only features their roles grant (union across all roles the user holds; e.g. only a user with the `admin` role sees DB Management). A user may hold multiple roles (B1.16).
...
sequenceDiagram
    ...
    Sg.App-->>Op: "Compute complete — scenario ready to run"
```
...
### 3.1 Standalone + Random mode execution (DRS Engineer, WS2-only)

Per [ADR-020](decision-record.md#adr-020--scenario-is-the-definition-exercise-is-the-run-drs-server-owned-random-mode-is-ws2-only) + [random-mode-execution-design](specs/random-mode-execution-design.md): Random mode is driven entirely by the **DRS Engineer** from the **drs_webapp on WS2**. `drs-server` writes the `exercises` run record and publishes the control message; **no `Sg.App`, no WS1, no STK.**
...
sequenceDiagram
    ...
    DRS->>Webapp: Configure Random ranges per variant (frequency / power)
    DRS->>Webapp: Start (Random + Standalone)
    Webapp->>drs-server: POST /exercises { mode:random, scope:standalone, variants:[{X, ranges}] }
    drs-server->>DB: INSERT exercises (mode=random, scenario_id NULL, status=executing)
    drs-server->>Kafka: Publish drs.session.control { action:start, exercise_id, mode:random, scope, variants:[{variant, ranges}] }
    drs-server-->>Webapp: 201 { exercise_id, status:executing }
    Kafka->>drs-bridge: Consume control
    drs-bridge->>drs-bridge: ResponseRouter → RandomGenerator(variant=X)
    loop Sustained for exercise duration
        drs-bridge->>drs-bridge: C++ generate_random(ranges) → bytes → parse → JSON
        drs-bridge->>Kafka: Publish hw.X.[kind]
        Kafka->>drs-server: Consume
        drs-server->>DB: INSERT telemetry hypertable (batched; stamped with exercise_id)
        drs-server->>Webapp: WebSocket broadcast → per-variant monitor-scan update
    end
    DRS->>Webapp: Stop
    Webapp->>drs-server: POST /exercises/{id}/stop
    drs-server->>Kafka: Publish drs.session.control { action:stop, exercise_id }
    drs-server->>DB: UPDATE exercises SET status=complete, ended_at=now()
    Kafka->>drs-bridge: Consume, RandomGenerator stops
```

**Notes:**
- No STK, no `Sg.App`, no WS1 — Random mode runs WS2-only; the SG workstation need not be up (ADR-020).
- `drs-server` is the writer of the `exercises` run record and the producer of `drs.session.control` (Random mode).
- Time reference is WS2-local (Random runs must not depend on WS1 as time server) — see [random-mode-execution-design §4.7](specs/random-mode-execution-design.md).
- Integrated + Random is identical except `drs-bridge` also TCP-outs to entity-controller apps.
...
sequenceDiagram
    ...
    Op->>Sg.App: Pick a COMPUTED scenario (compute_state=COMPUTED)
    Op->>Sg.App: Start Exercise (Scenario + Standalone)
    Sg.App->>drs-server: POST /exercises { mode:scenario, scope:standalone, scenario_id=S }
    drs-server->>DB: INSERT exercises (mode=scenario, scenario_id=S, status=executing)
    drs-server-->>Sg.App: 201 { exercise_id=X }
    Note over Sg.App: Sg.App now holds the run id X (bound to its scenario S)<br/>and can host GET /exercises/X/responses
    Sg.App->>Kafka: Publish drs.session.control { action:start, exercise_id:X, mode:scenario, scope:standalone }
    ...
    loop For each tick T (1 Hz default)
        ...
        Sg.App->>Sg.App: map active exercise X → its scenario_id S
        Sg.App->>DB: SELECT FROM computed_links WHERE (scenario_id=S, group_id, unit_id, tick)
...
sequenceDiagram
    ...
    Sg.App->>Kafka: Publish drs.session.control { action:start, exercise_id:X, mode:scenario, scope:integrated }
...
sequenceDiagram
    ...
    alt Pause
        ...
        Sg.App->>Kafka: Publish drs.session.control { action:stop, exercise_id:X }
...
sequenceDiagram
    ...
    drs-server->>drs-bridge: Roster change on `drs.roster` (new host:port) → hot-reload variant X (per drs-instance-addressing)
...
sequenceDiagram
    ...
    drs-server->>Kafka: Publish drs.admin { action:restart_parser, variant:X }
    drs-server-->>Webapp: 202 Accepted (async — watch live health)
    Kafka->>drs-bridge: AdminConsumer consumes
    drs-bridge->>drs-bridge: Stop variant X TCP server, flush buffers; reload parser .dll; restart server
    drs-bridge->>Kafka: Publish system.health { variant:X, status:healthy }
    Kafka->>drs-server: Consume → WS health push to webapp
    Webapp-->>DRS: Live health flips healthy; toast "Variant X parser restarted"
```

> **Bridge-lifecycle admin transport ([ADR-020](decision-record.md#adr-020--scenario-is-the-definition-exercise-is-the-run-drs-server-owned-random-mode-is-ws2-only) follow-on).** Per-variant *reload* (network/host:port change) rides the `drs.roster` hot-reload (§4.3). Per-variant *in-place restart* (§4.5 — reload a cached parser `.dll`, or kick a wedged server) is a distinct lifecycle action carried on a dedicated **`drs.admin`** topic (drs-server → drs-bridge; consumed by an AdminConsumer), separate from `drs.control` (bridge→server), `drs.session.control` (run-control), and `drs.roster` (addressing). Contract: [`contracts/kafka/drs.admin.schema.json`](../../contracts/kafka/drs.admin.schema.json). Completion is observed via `system.health` (the action is async). Topic provisioning + the AdminConsumer are implementation (owner A).
...
sequenceDiagram
    ...
    alt In-memory cache (if Phase 4/5 gates triggered cache mitigation)
        ...
        Sg.App->>DB: SELECT FROM computed_links WHERE (scenario_id, group_id, unit_id, tick) = (S, G, U, T)<br/>(S = active exercise X's scenario_id)
...
sequenceDiagram
    ...
    Sg.App->>Backend: ComputeLinks(scenarioId)
    ...
    Backend->>DB: UPDATE scenarios SET compute_state (unchanged — compute failed; no exercise involved)
    ...
    Sg.App->>Backend: Reload scenario from content_json (DB-of-truth; §2.7)
...
```mermaid
sequenceDiagram
    actor Admin as 👤 Admin (user holding the admin role)
    participant Sg.App
    participant drs-server
    participant DB
    ...
    alt Create user
        ...
        Admin->>Sg.App: Fill username, password (hashed client-side optional), assign one or more roles
        Sg.App->>drs-server: POST /admin/users
        drs-server->>DB: INSERT users + role bindings
        DB-->>drs-server: id
        drs-server-->>Sg.App: 201 Created
    else Modify user (role change)
        Admin->>Sg.App: Edit user, grant/revoke role bindings (e.g. grant the `admin` role)
...
sequenceDiagram
    ...
    drs-server->>DB: Ordered DELETE in FK order (reports → computed_links → telemetry → exercises → snapshots → scenarios → EW Library/system) + reset sequences
