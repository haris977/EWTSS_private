...
| X3 | Team Rust skill gap (Tauri) | Tauri not selected. Hybrid uses .NET / WPF (existing C# expertise) for Mode A and React + CesiumJS for Mode B (per ADR-009, unified with the React DRS webapp). |
...
| D2 | Drift between C# `Sg.Domain` DTOs and the JavaScript (JSDoc) mirror types in `Sg.Web` / drs-webapp — heightened by dropping TypeScript; mitigated by OpenAPI as contract source + JSON round-trip tests + an optional `checkJs` gate (tracked: [B1.45](design-backlog.md)) | Mode B Phase 2 kicks off | Frontend developer |
