...
| Layer | What |
|---|---|
| L6 Presentation | Sg.App (WPF, WS1) + DRS webapp (React, WS2) |
...
**Hybrid framing (ADR-001):** today = **Mode A** (WPF + STK ActiveX in-process). Future opt-in = **Mode B** (React+Cesium SPA + ASP.NET Core). Shared Sg.Domain. Saves cost today, preserves optionality.
...
|---|---|---|
| §1.1 SG | Scenario authoring with EW emitter library | Sg.App + Sg.Domain + StkScenarioBackend (Mode A); future Mode B via React SPA |
