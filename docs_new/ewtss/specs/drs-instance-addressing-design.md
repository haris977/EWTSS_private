...
> **Supersedes the earlier schema sketch.** The `roster` / `roster_entry` / `roster_revision` tables replace the per-instance-addressing sketch in [Developer Handbook §10.3](../developer-handbook.md) (`ip_configurations`), and the per-variant `parser_lib` / `port` / `kafka_topic` columns of `hardware_profiles` are superseded by the drs-bridge YAML template (Plan 1) + this roster. Non-addressing variant metadata (`protocol_version`, `config_json`) may remain in `hardware_profiles`.
...
> **Implementation status:** the DB store + CRUD/export-import endpoints landed in [Plan 2a](../plans/drs-roster-store-plan.md) (SQLAlchemy 2.0 async + Alembic; `roster`/`roster_entry`/`roster_revision`). The `drs.roster` **publisher** (compacted, key `active`, on every write + at startup), the `system.health` **presence consumer**, the **`/exercise/readiness` reconcile** (`configured`/`reachable`/`time_synced` + drift), and the **port-allocation engine** landed in [Plan 2b](../plans/drs-roster-reconcile-plan.md). Only the Sg.App + DRS-webapp surfaces (Plan 3) remain.
