...
| `drs-webapp` | DRS Engineer browser SPA | React 18 + Vite + JavaScript (JSX) | G |
| **`ewtss-release`** | Canonical docs, infra scripts, integration tests, release manifests, DVD installer, compatibility matrix | Markdown + PowerShell + Python | D (technical) + project lead (release approval) |
...
| `drs-webapp` | `npm ci` + `npm run build` + `npm test` (Vitest) on `ubuntu-latest` |
...
| React (`drs-webapp`) | Vitest + React Testing Library (`npm test`, jsdom) | `fetch` spied via `vi.spyOn`; TanStack Query with retries off; HTTP fully mocked |
...
- `infrastructure/` (NTP scripts + native Kafka KRaft broker docs + kafka topic provisioner)
- `packages/` (vendored installers)
- `.github/disabled/ci.yml` (parked CI workflow)
- `.githooks/` (settings-hygiene pre-commit guard, §9.2) + `.gitattributes` (LF-pins the hooks)
...
- **Permissions (`allow`)** — portable dev-tool wildcards only, no machine paths:
  `git`, `gh`, `dotnet`, `npm`/`npx`/`ng`/`node`, `cmake`, the `py`
  ...
  This rule is **mechanically enforced**: a committed `.githooks/pre-commit` guard
  (enabled per clone with `git config core.hooksPath .githooks`) blocks any commit
  whose staged `settings.json` carries a user-home path, or a `Bash`/`PowerShell`
  command that is neither a `tool:*` wildcard nor a short allow-list of portable
  bare commands — catching both file-targeted and ad-hoc one-offs. Those belong in
  `settings.local.json`. Every repo inherits this hook. The same check also runs
  server-side as the `settings-hygiene` CI job (parked with the rest of CI until
  hosted runners return), so the gate holds even for clones that never enabled
  the hook.
...
- **Test-coverage gaps against the §6.1 convention** (each wants a graceful-skip integration test added):
  ...
  - **`drs-webapp` coverage is unenforced and there is no browser e2e** — no Vitest coverage threshold is configured, and there is no Playwright smoke. Decide on a coverage gate plus a thin "dashboard renders against a mocked drs-server" e2e for the Phase 6 surfaces.
