...
**Row identity + IRS addressing (ADR-020).** Each `computed_links` row carries `entity_id`/`emitter_id` = the stable per-instance `id`s from `content_json` ([scenario-content-json-schema.json](../../../contracts/scenario-content-json-schema.json); opaque, not DB FKs) for SG-side display/scrubbing, plus `group_id`/`unit_id` — the IRS addressing the publisher matches on. `group_id`/`unit_id` are **ICD-derived deterministically at compute**: `emitter.instance_binding` (the `variant#n` logical id bound author-time per [drs-instance-addressing D4](drs-instance-addressing-design.md)) → variant → its ICD gives the RF-response Group and the response operation's Unit ([icd-reference-comm-df §4](../icd-reference-comm-df.md)). They are not operator-entered. A scenario whose DRS-driven emitters lack an `instance_binding` cannot produce addressable link rows — the compute/readiness path surfaces that as a binding gap.
...
- Archive is a soft-delete state (`archived_at IS NOT NULL`); the row is preserved indefinitely so reports that reference its snapshots — and `exercises` rows that reference the scenario ([ADR-020](../decision-record.md#adr-020--scenario-is-the-definition-exercise-is-the-run-drs-server-owned-random-mode-is-ws2-only)) — remain interpretable. Both FKs also block a bare hard delete (hard delete is reserved for the ordered DB Purge, §4.9). Archived scenarios are hidden from the library UI list by default; visible via "Show archived" filter.
...
**Concurrent-edit-during-execution:** if `scenario_id` is the scenario of a currently-executing exercise, the Save button is disabled at the UI level. Operator must Stop the exercise first. The authoritative execution state is the `exercises` table (drs-server-owned, [ADR-020](../decision-record.md#adr-020--scenario-is-the-definition-exercise-is-the-run-drs-server-owned-random-mode-is-ws2-only)): a scenario is executing iff a row exists with `scenario_id = X AND status = 'executing'`. `Sg.App`'s `IExerciseStateService` mirrors that state (it requested the run and holds the `exercise_id`) for the client-side guard; drs-server remains the source of truth.
...
**Reports are a Scenario-mode capability only.** `scenario_id` and `snapshot_id` are `NOT NULL`, so a report always corresponds to a computed scenario. A **Random-mode** exercise has neither ([ADR-020](../decision-record.md#adr-020--scenario-is-the-definition-exercise-is-the-run-drs-server-owned-random-mode-is-ws2-only)) and is **non-reportable** by design — its post-run evidence is the captured telemetry (`measurements`), reviewed in the drs_webapp. drs-server rejects a report request for a `mode='random'` exercise. (If formal Random-run reports are ever required, that is a schema change — `reports` keyed on `exercise_id` with nullable scenario/snapshot — tracked in the reports B1.x item.)
...
Scenarios reference EW Library templates (emitters, antennas, etc.) by template ID. The `content_json.compute_inputs.emitters` array stores `{ id, template_id, instance_binding?, instance_overrides: {...} }` objects, where `id` is the **stable per-instance identity** (uuid, referenced by `computed_links.emitter_id`) and `instance_binding` is the optional DRS `variant#n` logical id (ADR-020); `template_id` is the shared EW Library template, **not** a per-instance key. EW Library schema is owned by [B1.7](../design-backlog.md); this spec assumes the template ID is a stable foreign-key target.
...
Per [B1.14](../design-backlog.md), the DB Purge admin action clears all scenarios + `scenario_compute_snapshots` + `computed_links` + `exercises` + telemetry + EW Library + reports. It is executed by **drs-server** (which owns all DDL and this ordered purge, per [ADR-020](../decision-record.md#adr-020--scenario-is-the-definition-exercise-is-the-run-drs-server-owned-random-mode-is-ws2-only) / [database-ownership-design](database-ownership-design.md)) as an **ordered DELETE in FK order** — not a bare `TRUNCATE` of hypertables, since `scenarios` / `scenario_compute_snapshots` / `exercises` / `reports` are regular tables and several FKs cross the scenario-planning ↔ execution/telemetry boundary. Because `exercises.scenario_id → scenarios` (ADR-020), the whole execution/telemetry chain must be cleared **before** `scenarios`:

1. `reports` (FK → snapshots, scenarios).
2. `computed_links` (FK → scenarios).
3. `measurement_scalars` → `measurements` → `drs_sessions` (telemetry; `drs_sessions.exercise_id → exercises`).
4. `exercises` (FK → scenarios).
5. `scenario_compute_snapshots` (FK → scenarios).
6. `scenarios`.
7. EW Library (`emitter_library`) + system tables (RBAC, roster, logs) — then re-seed master data.

Foreign-key constraints prevent partial purge — if any step fails, the whole operation rolls back. Confirmation UX in [B1.14](../design-backlog.md).
