...
Separate React project (per [ADR-009](../decision-record.md#adr-009--browser-spa-framework-mode-b-react--cesiumjs)). Talks to `Sg.Server` via REST + SignalR. Consumes CZML for visualization. Shares no code with the WPF app, but consumes the same JSON DTOs (potentially via a TypeScript types file generated from the C# contracts).
