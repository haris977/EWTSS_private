...
- **WS1** — SG (Scenario Generator): `Sg.App` (WPF) + STK 12, the SG Operator's workstation; the NTP Stratum-1 time server **when present**.
- **WS2** — DRS: `drs-server`, `drs-bridge`, the DRS webapp (browser), talking to DRS hardware instances on the LAN. WS2 carries its **own local time reference** so DRS-Engineer, WS2-only Random exercises remain runnable with WS1 powered off ([ADR-020](../decision-record.md#adr-020--scenario-is-the-definition-exercise-is-the-run-drs-server-owned-random-mode-is-ws2-only); see [random-mode-execution-design](random-mode-execution-design.md)).
...
**Role catalogue (B1.16, resolved).** Three roles — **Admin**, **SG Operator**, **DRS Engineer** (no `observer`). A **user may hold multiple roles** (effective permissions = union), so a `user_roles` join table replaces the single `users.role_id` FK. The `roles.name` enum in [developer-handbook §10.3](../developer-handbook.md#10-database-schema-reference) is updated to `admin | sg_operator | drs_engineer`. Shared RBAC store on WS2 PostgreSQL; `role_features` maps roles → features.

**Admin is least-privilege**, not a superuser: Admin holds user/role management, system admin (DB purge, backup/restore, time-sync, hotfix), and view — but **not** authoring or exercise-run. Operational authority is granted by *also* holding the SG Operator and/or DRS Engineer role.

| Feature | Admin | SG Operator | DRS Engineer |
|---|:--:|:--:|:--:|
| `scenario.author` (author scenarios / EW Library / map / compute) | – | ✓ | – |
| `exercise.run.scenario` (start/stop Scenario-mode exercise, WS1) | – | ✓ | – |
| `exercise.run.random` (start/stop Random-mode exercise, WS2) | – | – | ✓ |
| `report.generate` (Scenario-mode reports) | – | ✓ | – |
| `telemetry.view` (live telemetry + DRS health) | ✓ | ✓ | ✓ |
| `drs.config` (IP / network / roster) | – | – | ✓ |
| `drs.restart` (restart TCP server / parser) | – | – | ✓ |
| `log.view` (4-class logs) | ✓ | ✓ | ✓ |
| `user.manage` (users / roles) | ✓ | – | – |
| `system.admin` (DB purge, backup/restore, time-sync, hotfix) | ✓ | – | – |

- **Exercise run-control is enforced server-side (ADR-020).** `POST /exercises` (drs-server writes the `exercises` row) checks `exercise.run.scenario` (Scenario) or `exercise.run.random` (Random) on the caller.
- **Rule:** every drs-server endpoint and every privileged Sg.App action checks the caller's permission server-side (union across the caller's roles); the UI hiding a control is never the only gate. A per-endpoint permission audit is part of the security pass ([B1.28](../design-backlog.md)).
