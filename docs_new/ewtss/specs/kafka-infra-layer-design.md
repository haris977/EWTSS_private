...
infrastructure/
  kafka/
    README.md                     ← native KRaft broker setup docs (NEW)
    create-topics.py              ← idempotent topic provisioning (NEW)
  ntp/                            ← unchanged
```

> **Update (post-B1.43-Plan-2a):** the originally-planned `infrastructure/docker-compose.yml`
> was removed — Docker is never used in the air-gapped Windows deployment, so the dev path
> runs the Kafka KRaft broker **natively** to match production. The component note below is
> retained for design context; the runnable instructions live in `infrastructure/kafka/README.md`.
...
**Local Kafka broker** — single Kafka KRaft broker (no Zookeeper, single-node
quorum), run natively (no Docker). Bind: `localhost:9092`. The KRaft log dir
persists topics across restarts; deleting it resets state. Liveness checked via
`kafka-topics --list`. Setup is in `infrastructure/kafka/README.md`.
...
| Omission | Reason |
|---|---|
| TimescaleDB service | (Updated post-B1.43-Plan-2a: `drs-server` now uses PostgreSQL for the roster store, installed natively for dev — see [infrastructure/kafka/README.md](../../infrastructure/kafka/README.md) and the top-level README "bring up local infra"; the `docker-compose` stack was removed since production is air-gapped Windows. *TimescaleDB-specific* hypertables still wait for the telemetry/measurements schema work.) |
...
- **Persistent vs ephemeral Kafka state?** Keep the KRaft log dir on disk so
  topics survive restarts; document that deleting it resets state. Devs prefer
  this for iterating on topic changes without re-creating state every time.
