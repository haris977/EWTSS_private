...
### 4.5 WS2 local time source — for WS2-only Random mode

Per [ADR-020](../decision-record.md#adr-020--scenario-is-the-definition-exercise-is-the-run-drs-server-owned-random-mode-is-ws2-only), **Random-mode** exercises are DRS-Engineer-driven and run **WS2-only, with WS1 potentially powered off**. Since WS1 is the Stratum-1 source (§4.2), a WS2-only run cannot depend on it for time. The discipline:

- **Local-clock fallback on WS2.** The WS2 Meinberg daemon (§4.3) is configured with an **undisciplined local-clock reference** at a high stratum (the standard NTP `LOCAL` / orphan pattern, e.g. stratum 10). While SG is reachable, WS2 disciplines to SG-Stratum-1 as normal; when SG is absent, WS2 falls back to serving a **coherent, monotonic local time** to its own services (drs-server, drs-bridge, webapp) rather than declaring hard sync-loss. No new component — this is a Meinberg config option.
- **This is the "own local time reference" that [random-mode-execution-design §4.7](random-mode-execution-design.md) and [deployment-guide §1](../deployment-guide.md) require.** RTC vs. local-NTP is a config detail; the local-clock-fallback approach above is the baseline.
- **Fidelity note.** WS2 local time has lower absolute accuracy than SG-Stratum-1-disciplined time — acceptable for Random (hardware-checkout / diagnostic), which has no WS1 cross-correlation requirement. IRS frame timestamps still derive from a single coherent WS2 clock, so intra-run ordering is intact.
...
`Sg.App` refuses to start a **Scenario-mode** exercise until all WS2 services report a healthy sync status. This avoids the failure mode where an exercise begins immediately after WS2 reboot while Meinberg is still converging.

For **WS2-only Random** exercises ([ADR-020](../decision-record.md#adr-020--scenario-is-the-definition-exercise-is-the-run-drs-server-owned-random-mode-is-ws2-only)), the `SyncStateEngine` evaluates health against **WS2's local reference** (§4.5), not WS1 reachability: a WS2-only Random run must **not** auto-pause merely because WS1 (the Stratum-1 upstream) is absent — that is the expected operating condition for Random. drs-server gates and starts the run; `Sg.App` is not involved.
...
- [Command Flows §5.4](../../ewtss/command-flows.md#54-time-synchronisation--sg-as-time-server-meinberg-ntp-per-b13) — initial sequence-diagram for SG-as-Time-Server (this design replaces the "protocol TBD" notes with concrete Meinberg + per-IRS adapter design).
