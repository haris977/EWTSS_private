...
**Decision:** Hybrid frontend. Mode A — `Sg.App.exe` (C# WPF + STK ActiveX in-process) — is the primary deliverable. Mode B — `Sg.Server.exe` (ASP.NET Core) + `Sg.Web` (React + CesiumJS SPA, per [ADR-009](#adr-009--browser-spa-framework-mode-b-react--cesiumjs)) — is an opt-in future addition that reuses the same `Sg.Domain` (DTOs, `IScenarioBackend`, `StkScenarioBackend`).
...
## ADR-009 — Browser SPA framework (Mode B): React + CesiumJS

**Status:** Decided in advance; activation deferred until Mode B is funded. **Revised 2026-06-25** — framework changed Angular → React + JavaScript to keep a single v2 web-frontend stack (see "Revision note" below).

**Decision:** When Mode B ships, the SPA is React written in **JavaScript** (matching the org's React-in-JavaScript convention) hosting a CesiumJS scene, served as static assets by `Sg.Server`. Loads CZML emitted by the server (Phase B1 read-only) or DTOs over REST (Phase B2 authoring). The C#→browser DTO contract is guarded by JSON round-trip tests + OpenAPI (and optional JSDoc typedefs), not a TypeScript compiler.

**Revision note (2026-06-25):** The DRS webapp (WS2) moved Angular → React + JavaScript per the [ADR-018](#adr-018--ws2-drs-webapp-required-browser-frontend-on-the-drs-workstation-served-by-drs-server) revision, driven primarily by the org's React teams standardising on JavaScript (which Angular forbids), plus an Angular-developer shortage and org-wide React momentum. The original ADR-009 pre-committed Mode B to Angular partly so the DRS webapp's component library (data tables, log viewers, filter/sort controls) would be partially reusable in `Sg.Web` ([v2 Execution Plan §3.7](v2-execution-plan.md#37-g-react--drs-webapp-lead)). Leaving Mode B on Angular after the DRS-webapp change would **forfeit that reuse and split the v2 web frontend into two toolchains** (two component libraries, two skill pools). Re-pointing Mode B to React preserves the reuse path (now on React) at **zero code cost** — `Sg.Web` is not yet built. CesiumJS is framework-agnostic and was validated for STK CZML rendering by MVP3 independently of the host framework; the Electron host ([ADR-010](#adr-010--browser-host-electron-mode-b-desktop-browser-deployment)) is unaffected by the framework choice.

**Why:** A single React web-frontend stack across both v2 browser surfaces (DRS webapp now, `Sg.Web` when funded) matches current team staffing and preserves DRS-webapp → `Sg.Web` component reuse. CesiumJS validated by MVP3 for STK CZML rendering; mature pipeline, framework-independent integration.

**Rejected:**
- **Angular + Cesium** (the original 2026-04 choice, on team-Angular-familiarity grounds) — superseded by the org's React shift and the DRS-webapp framework change; keeping Mode B on Angular would split the v2 web frontend into two toolchains and forfeit the planned component-library reuse.
...
**Refs:** v2 §8, §22.4; [ADR-018](#adr-018--ws2-drs-webapp-required-browser-frontend-on-the-drs-workstation-served-by-drs-server); [v2 Execution Plan §3.7](v2-execution-plan.md#37-g-react--drs-webapp-lead).
...
**Status:** Accepted 2026-05-13. **Framework call made 2026-06-25 — React** (see "Framework decision" below).

**Decision:** WS2 hosts a browser-based DRS webapp, served by `drs-server`, for the DRS Engineer persona. The webapp is independent of the SG-side Mode A / Mode B selection — it is present in every deployment. Framework: **React with JavaScript (JSX)**, built with Vite, tested with Vitest + React Testing Library, styled with SCSS. The original ADR left this as "Angular preferred / React acceptable fallback" with the call due at end of week 1 of hardening; that call is now made — see below.

**Framework decision (2026-06-25 — React + JavaScript):** The week-1 framework gate resolved to **React, written in JavaScript**, exercising the "React acceptable fallback" clause the original ADR left open for exactly this contingency. Drivers, in priority order: (1) **the company's React teams standardise on JavaScript, and Angular mandates TypeScript** — staying on Angular would have forced a language the available React bench does not use day-to-day; (2) an Angular-developer shortage on the team; (3) org-wide React momentum (most other active projects are React). The two original pro-Angular arguments were skill alignment with E (the SG-side frontend developer, whose prior stack was Angular) and future component-library reuse with the Mode B `Sg.Web` SPA; the staffing/language reality inverts the first, and the second is preserved by re-pointing Mode B itself to React + JavaScript ([ADR-009](#adr-009--browser-spa-framework-mode-b-react--cesiumjs) revised the same day) — so the v2 web frontend stays a **single React + JavaScript stack** across both browser surfaces rather than splitting into two toolchains. Cost was near-zero: the webapp was a fresh scaffold with one feature slice (the Time Sync surfaces), re-scaffolded to Vite + React + JavaScript with the slice ported and tests green. `drs-server`'s static-serving contract is unchanged — the Vite build still emits to `dist/drs-webapp/`.

**Trade-off (JavaScript, not TypeScript):** Plain JS forgoes compile-time checking on the DTO / contract boundary that v2's architecture leans on (the C# `Sg.Domain.Contracts` DTOs the webapp consumes over REST/WS). This is accepted to match the org's React-in-JavaScript convention, and mitigated by: JSDoc `@typedef`s on the payload shapes (editor-level hints, no build commitment); the existing C#-side JSON round-trip tests and OpenAPI as the contract source of truth; and an optional `checkJs` type-check gate for stricter enforcement (writing JS, checked via JSDoc — though a CI gate re-adds dev-only `tsc`/`@types` tooling). That `checkJs` decision is deferred and tracked as [Design Backlog B1.45](design-backlog.md), not assumed.
...
- Team grows from 6 to 7: a dedicated DRS-side frontend developer (G, React) is on the team, owning the webapp surfaces across the 17-week build per [v2 Execution Plan §3.7](v2-execution-plan.md#37-g-react--drs-webapp-lead).
...
- **Vue or Svelte for the webapp framework.** Not rejected on technical grounds, but the choice was between Angular and React (the two stacks with team/org skill behind them); React was selected on 2026-06-25 (see "Framework decision" above).
- **Angular for the webapp.** The original preference (skill alignment with E + Mode B component reuse); reversed on 2026-06-25 by the Angular-developer shortage and org-wide React momentum, with Mode B re-pointed to React ([ADR-009](#adr-009--browser-spa-framework-mode-b-react--cesiumjs)) to keep one frontend stack.

**Refs:** RFQ Annexure A.1 §B + §F + §2.5; [Operator Playbook §9](operator-playbook.md#9-drs-engineer-workflows-ws2-webapp); [Architecture Overview §3.9](architecture-overview.md#39-drs-webapp--drs-engineer-surface-ws2); [v2 Execution Plan §3.7](v2-execution-plan.md#37-g-react--drs-webapp-lead); [Design Backlog B1.1 + B1.2](design-backlog.md#1-milestone-1-design-items).
...
## ADR-020 — Scenario is the definition; exercise is the run (drs-server-owned); Random mode is WS2-only

**Status:** Accepted 2026-07-15.

**Decision:** Separate two concepts that the early relational schema had conflated:

- **Scenario** = the authored, saved, *computed definition* (library artifact). Its definition lives in `scenarios.content_json`; its STK link-analysis is computed once and stored in `computed_links` keyed on **`scenario_id`** (`entity_id` / `emitter_id` on those rows are opaque logical ids drawn from `content_json`, not DB foreign keys). Written by `Sg.App`.
- **Exercise** = a *run instance* of a scenario (Scenario mode) or of a live Random configuration (Random mode). The `exercises` row is the run record and the parent of the telemetry tables (`drs_sessions` → `measurements`). It carries `scenario_id` **nullable** — null means a Random-mode run with no scenario. It lives in the **execution/telemetry domain and is written by `drs-server` for both modes.** `Sg.App` never writes `exercises`; for a Scenario run it requests start over the existing Kafka `drs.session.control`, and `drs-server` mints the row.
- **Random mode is fully WS2-driven.** The **DRS Engineer** configures parameter ranges and starts/stops the run from the drs_webapp; `drs-server` publishes `drs.session.control` and writes the `exercises` row; `drs-bridge` generates. No STK, no scenario, no `computed_links`, no `Sg.App` — **the SG Operator and the SG workstation need not be running.**

**Why:**
- The doc set already treated scenario (authoring) and exercise (execution) as distinct — the conflation existed only in the pre-B1.19 relational schema in [developer-handbook §10.1](developer-handbook.md#10-database-schema-reference), which [scenario-management-design](specs/scenario-management-design.md) superseded by moving the definition into `content_json` without cleaning up §10.1.
- Random needs no STK / scenario / `computed_links`, and its control-plane producer was *already* `drs-server` (`drs.session.control`). The only WS1 coupling was the *location of the config UI*. Removing it eliminates an availability dependency: hardware checkout and Random exercises are DRS-Engineer operations that must not require the SG workstation to be up.
- `exercises` is a run concept whose children (`drs_sessions` → `measurements`) are drs-server-written telemetry. Co-locating the run-record writer with the telemetry writer is cohesive and is what lets Random run WS2-only.

**Implications:**
- [developer-handbook §10](developer-handbook.md#10-database-schema-reference) reconciled: `exercises` moves to the telemetry/execution domain; the per-scenario definition child tables (`entities`, `emitters`, `emitter_parameters`, …) are removed (superseded by `content_json`); `emitter_library` stays (cross-scenario EW Library, B1.7); `computed_links` re-pointed to `scenario_id`.
- Scenario publisher interface stays **`exercise_id`-facing** (`GET /exercises/{id}/responses`) — the bridge references the run it is serving and never learns the scenario concept. The publisher **resolves the active exercise → its `scenario_id` internally** and queries `computed_links` by `scenario_id` (compute-once storage, replayed across runs). Storage keying moved from `exercise_id` to `scenario_id`; the *interface* is unchanged.
- Four-mode matrix: both Random rows are **WS2-only** (WS1 not required).
- The drs_webapp gains a Random-mode configuration + run-control surface. Its execution flow (drs-server endpoints, `exercises` write, `drs.session.control` publishing) is specced in [random-mode-execution-design](specs/random-mode-execution-design.md); the visual UI layout is a post-[B1.1](design-backlog.md) wireframe item. Backlog: [B1.9](design-backlog.md) (re-homed to the DRS side).
- Table ownership is recorded in [database-ownership-design §4](specs/database-ownership-design.md): `exercises` → `drs-server`.

**Rejected:**
- **Collapse `exercise` into `scenario` (a scenario *is* its run).** Cannot represent a Random-mode run (no scenario) or repeat-runs of one scenario, and would force per-run telemetry to key on a library artifact.
- **`Sg.App` writes `exercises` / SG required for all exercises.** Couples hardware checkout and Random exercises to WS1 availability and contradicts the DRS-Engineer persona owning hardware-side operation ([ADR-018](#adr-018--ws2-drs-webapp-required-browser-frontend-on-the-drs-workstation-served-by-drs-server)).

**Refs:** [database-ownership-design](specs/database-ownership-design.md); [scenario-management-design §4.1](specs/scenario-management-design.md); [developer-handbook §10](developer-handbook.md#10-database-schema-reference); [architecture-overview §4.6 + §4.8](architecture-overview.md); [operator-playbook §8 + §9](operator-playbook.md).
...
| 2026-06-25 | ADR-018 framework call made — **React + JavaScript** (org React teams standardise on JavaScript, which Angular forbids; + Angular-developer shortage + org React momentum); ADR-009 revised Angular → React + JavaScript to keep a single v2 web-frontend stack. DRS webapp re-scaffolded to Vite + React + JavaScript. |
