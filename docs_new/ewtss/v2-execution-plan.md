...
- Build the **DRS webapp** (browser SPA on WS2 served by drs-server — React, per [ADR-018](decision-record.md#adr-018--ws2-drs-webapp-required-browser-frontend-on-the-drs-workstation-served-by-drs-server)).
...
| **E** | SG-side frontend developer | React (SPA frontend) — cross-trained to Python during ramp | Weeks 1–2: Python ramp + take ownership of `drs-server` REST query endpoints + RBAC + JWT auth from week 3. Joins the React `Sg.Web` build when Mode B activates (post-ship). | Python 0.7 (after week 2 ramp) |
| **G** | DRS-side frontend developer | React | `DRS webapp` lead — browser SPA on WS2, served by `drs-server`. Health dashboard, per-variant monitor-scan, IP / network configuration, message logs, per-variant control. Framework decided at Milestone-1 design — **React** ([ADR-018](decision-record.md#adr-018--ws2-drs-webapp-required-browser-frontend-on-the-drs-workstation-served-by-drs-server)), unified with the Mode B `Sg.Web` stack ([ADR-009](decision-record.md#adr-009--browser-spa-framework-mode-b-react--cesiumjs)). | Frontend 1.0 |
...
- Frontend developer (React / Cesium) — covered by E when Mode B activates (E is a React developer, cross-trained to Python during hardening); no separate hire needed (existing in-house React skill is the de-risking that the [Risk Register R8](risk-register.md) calls out).
...
- Scenario publisher HTTP endpoint: `GET /exercises/{id}/responses?group_id=X&unit_id=Y&tick=Z` (exercise-facing; maps to `scenario_id` internally and reads `computed_links` by `scenario_id` — [ADR-020](decision-record.md#adr-020--scenario-is-the-definition-exercise-is-the-run-drs-server-owned-random-mode-is-ws2-only)) returning the pre-computed scenario response from `computed_links`. Hosted from `Sg.App` itself during exercise execution; `drs-bridge`'s `ResponseRouter` consumes it in SCENARIO mode.
- Compute → write-to-TimescaleDB pipeline: `IScenarioBackend.ComputeLinks(scenarioId)` runs STK link-analysis tick-by-tick, batches results, writes to PostgreSQL on WS2 over LAN. Already specified in [Architecture Overview §4.5](architecture-overview.md#45-scenario-compute-and-link-analysis-write-back-scenario-modes-planning).
...
### 3.6 E (React, cross-trained) — `drs-server` REST + auth + reports
...
- Joins the React `Sg.Web` frontend build when Mode B activates (post-acceptance).

### 3.7 G (React) — DRS webapp lead

- **Weeks 1–2: Framework decision + scaffolding.** Framework confirmed **React in JavaScript** (Vite, Vitest + React Testing Library, SCSS) per [ADR-018](decision-record.md#adr-018--ws2-drs-webapp-required-browser-frontend-on-the-drs-workstation-served-by-drs-server). Set up build pipeline; produce a "hello-world" SPA served by drs-server at `/`. Pair with F + E on the REST + WebSocket contract design so DRS webapp endpoints are scoped into the same OpenAPI surface (separate `/webapp/...` prefix or shared namespace — Milestone-1 design call).
...
- **When Mode B activates (post-ship):** G + E together on the Mode B SG-side SPA (`Sg.Web`). Because both surfaces are React ([ADR-009](decision-record.md#adr-009--browser-spa-framework-mode-b-react--cesiumjs)), the DRS webapp's React component library is partially reusable for Mode B (data tables, log viewers, filter / sort controls).
...
[G]   DRS webapp framework decision (React, per ADR-018) signed
...
[A]   RandomGenerator: per-hardware random data within YAML-declared
      ranges. drs.session.control topic for start/stop/config.
...
| **CI bring-up harness** | Phase 1 | Spins up Kafka KRaft + PostgreSQL + drs-server + drs-bridge natively (no Docker — CI uses workflow service-containers) for the integration test environment |
...
| **DRS webapp framework decision slips past week 2** — G's scaffolding cannot begin until the framework is signed off; every week of delay compresses G's downstream surfaces (monitor-scan, log viewer, IP config) against the back end of the build | G + D + project lead | **Resolved 2026-06-25: React + JavaScript** ([ADR-018](decision-record.md#adr-018--ws2-drs-webapp-required-browser-frontend-on-the-drs-workstation-served-by-drs-server)); scaffold landed (Vite + React + JavaScript). Decision gate was end of week 1 with project-lead final call; the org's React-in-JavaScript convention + Angular-developer shortage made React the call with no further escalation |
| **G — DRS webapp scope creep** — customer requests additional WS2-side surfaces (custom report builder, multi-variant comparison views, real-time alerting) mid-build | G + project lead | Lock weeks-12-onwards scope at end of week 11; new surfaces are chargeable additions per [Risk Register P3](risk-register.md); G's React component library makes additive views ~1 day each, but each one is logged and estimated |
...
| **Contracts frozen** | 2 | C++ parser ABI signed off; YAML profile schema signed off; `IScenarioBackend` extensions signed off; OpenAPI spec for `drs-server` signed off (including DRS-webapp endpoint surface); DRS webapp framework decision (React) signed off; E's Python ramp on track | Phase 1 test passes (§6.2) |
