...
- **L6 — Presentation.** Two surfaces, one per operator persona. **SG-side** on WS1 for the Scenario Operator: today `Sg.App` (C# WPF + STK ActiveX in WindowsFormsHost); future opt-in `Sg.Web` (React SPA + CesiumJS). **DRS-side** on WS2 for the DRS Engineer: a browser-based webapp served by `drs-server` (React per [ADR-018](decision-record.md#adr-018--ws2-drs-webapp-required-browser-frontend-on-the-drs-workstation-served-by-drs-server), decided 2026-06-25). Both bind through L5.
...
**No SG-side browser delivery today.** The SG frontend's browser path (Mode B: React + CesiumJS SPA + ASP.NET Core server) is fully designed and ~90% of the contract-boundary plumbing has already shipped, but the actual server and SPA are deferred until a customer asks. We can deliver Mode B in ~1.5 months (read-only viewer) + ~3 months (full authoring) once funded — but if no customer ever asks, that capability lives as cold code. **The DRS-side webapp on WS2 is required in every deployment** and is separate from the Mode A / Mode B selection for the SG frontend.
...
**1. Mode B (browser delivery) activation.** ~90% of the work to enable Mode B already shipped in MVP4.5 — the DTO contracts, the `IScenarioBackend` boundary interface, the JSON round-trip tests, the namespace-fence integration test that fails the build if a COM type leaks into the contracts namespace. The remaining 10%: `Sg.Server` (ASP.NET Core mapping endpoints to interface methods), `Sg.Web` (React + CesiumJS SPA mirroring the C# view-models), the OpenAPI contract, GIS tile pipeline. Estimated 1.5 months (read-only viewer) + 3 months (full authoring). Activated on customer signal; the C# track does not pay this cost twice. Detailed activation checklist in [Architecture Overview §9.1](architecture-overview.md#91-mode-b-activation-checklist).
...
**Headline:** v2 hardening is **17 weeks, 7 engineers**, ~4 months of calendar time from kickoff to ship. Detailed staffing and per-person ownership in [v2 Execution Plan §2 + §3](v2-execution-plan.md#2-team-composition). The team is already in place: one senior Python developer (F), one Python+C# polyglot (A), one C# specialist (B), one C++ specialist (C), one cross-stack lead / reviewer (D), one React developer cross-trained to Python during ramp (E, SG-side), one React developer for the DRS webapp on WS2 (G).
...
3. **DRS webapp framework decision slips past week 2.** G's scaffolding cannot begin until the framework is signed off; every week of delay compresses G's downstream surfaces (monitor-scan, log viewer, IP config) against the back end of the build. **Resolved 2026-06-25: React** ([ADR-018](decision-record.md#adr-018--ws2-drs-webapp-required-browser-frontend-on-the-drs-workstation-served-by-drs-server)); scaffold landed. The gate was end of week 1 with project-lead final call; the Angular-developer shortage + org React momentum settled it on React.
...
5. **The phased integration test rig with cumulative regression.** Phase N's test = Phase N's new scope + every prior phase's test, run on a clean freshly-provisioned environment, gated before forward motion. The rig structure (SDFC simulator + synthetic Kafka producer + multi-instance simulator + STK fixtures) is reusable wholesale for any system with multiple cooperating services and external hardware simulation. Adopting this discipline early on a new project surfaces architectural problems before they are load-bearing.
