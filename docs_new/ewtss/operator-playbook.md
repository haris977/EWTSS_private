...
|---|---|---|---|
| **SG Operator** (Scenario Operator) | WS1 — Scenario Generator | `Sg.App` C# WPF + STK ActiveX (Mode A, today) — future opt-in: `Sg.Web` React SPA (Mode B) | Authors scenarios, manages the EW library, computes link analysis, runs **Scenario-mode** exercises, generates reports, and views logs. (User/system administration is the **Admin** role, held additionally — B1.16 least-privilege split.) |
| **DRS Engineer** | WS2 — DRS (Device Replacement Software) | DRS webapp served by `drs-server` (browser-based; React per [v2 Execution Plan §3.7](v2-execution-plan.md#37-g-react--drs-webapp-lead) / [ADR-018](decision-record.md#adr-018--ws2-drs-webapp-required-browser-frontend-on-the-drs-workstation-served-by-drs-server)) 🚧 | Monitors hardware health and per-variant live data, configures hardware IP / network parameters, reviews sent/received message logs, performs DRS-side diagnostics, and **configures + runs Random-mode exercises** (WS2-only; no SG workstation required — [ADR-020](decision-record.md#adr-020--scenario-is-the-definition-exercise-is-the-run-drs-server-owned-random-mode-is-ws2-only)). |
...
5. **The exercise is run.** For **Scenario mode**, the SG Operator selects Standalone/Integrated and starts execution on WS1; the computed link-analysis is replayed tick-by-tick. For **Random mode**, this is a **DRS-Engineer, WS2-only** operation driven from the drs_webapp (no SG Operator, no WS1) — synthetic data is generated from DRS-Engineer-configured parameter ranges ([ADR-020](decision-record.md#adr-020--scenario-is-the-definition-exercise-is-the-run-drs-server-owned-random-mode-is-ws2-only)).
...
> **Who drives which mode ([ADR-020](decision-record.md#adr-020--scenario-is-the-definition-exercise-is-the-run-drs-server-owned-random-mode-is-ws2-only)):** **Scenario-mode** exercises are run by the **SG Operator** on WS1 (the scenario is authored + computed there, and `Sg.App` publishes ticks during execution). **Random-mode** exercises are run by the **DRS Engineer** from the **drs_webapp on WS2** and require **no SG workstation** — no scenario, no STK, no `Sg.App`. The steps in §8.1–§8.2 below describe the Scenario-mode (SG Operator) flow; the Random-mode equivalent lives on the DRS-Engineer surface (§9).

### 8.1 Exercise setup (Scenario mode — SG Operator)
...
| 4 | If Random: this is the **DRS Engineer's** flow on WS2 — configure per-variant parameter ranges (frequency min/max, power range, etc.) and run from the drs_webapp; the SG workstation is not involved (see §9, [ADR-020](decision-record.md#adr-020--scenario-is-the-definition-exercise-is-the-run-drs-server-owned-random-mode-is-ws2-only)). | A.1 §A Random Mode | Random configuration + run-control view in the drs_webapp | 📐 Design gap |
...
The DRS Engineer's surface is a **browser-based webapp on WS2**, served by drs-server. Framework: React in JavaScript (Vite) per [v2 Execution Plan §3.7](v2-execution-plan.md#37-g-react--drs-webapp-lead) / [ADR-018](decision-record.md#adr-018--ws2-drs-webapp-required-browser-frontend-on-the-drs-workstation-served-by-drs-server), decided 2026-06-25. Owner: G. The persona's responsibility is hardware-side health, configuration, and diagnostics — distinct from the SG Operator's scenario-authoring role.
...
| 10 | Engineer configures **Random-mode parameter ranges** per variant (frequency min/max, power range, etc.) for a hardware-checkout / diagnostic run. | A.1 §A Random Mode | DRS webapp Random-configuration view | 📐 Design gap ([ADR-020](decision-record.md#adr-020--scenario-is-the-definition-exercise-is-the-run-drs-server-owned-random-mode-is-ws2-only)) |
| 11 | Engineer **starts / stops a Random-mode exercise** (Standalone or Integrated) — WS2-only, no SG workstation required. drs-server writes the `exercises` run record and publishes `drs.session.control`. | A.1 §A | DRS webapp run-control panel | 📐 Design gap ([ADR-020](decision-record.md#adr-020--scenario-is-the-definition-exercise-is-the-run-drs-server-owned-random-mode-is-ws2-only)) |

Detailed UX (specific widget layouts, navigation patterns, accessibility primitives) is part of Milestone-1 Mock UI Wireframes — see [v2 Execution Plan §3.7 / §4.1](v2-execution-plan.md#37-g-react--drs-webapp-lead).

---

## 10. Reports

> **Scenario-mode only.** Reports correspond to a computed scenario (they require a `scenario_id` + compute snapshot). **Random-mode** exercises are non-reportable by design ([ADR-020](decision-record.md#adr-020--scenario-is-the-definition-exercise-is-the-run-drs-server-owned-random-mode-is-ws2-only)); a DRS Engineer's post-run evidence for a Random run is the captured telemetry, reviewed live in the drs_webapp (§9). Reports remain an SG-Operator / `Sg.App` surface.
...
The DRS webapp surfaces themselves (§9) are no longer open design questions — G owns them and they are scoped week-by-week in [v2 Execution Plan §3.7](v2-execution-plan.md#37-g-react--drs-webapp-lead). What remains for those surfaces is detailed UX wireframes (B1.1).
