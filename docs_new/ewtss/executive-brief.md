...
A next-generation EWTSS (Electronic Warfare Training Software System) that:
...
| SG-side authoring frontend (future, opt-in) | **React + CesiumJS** browser SPA — `Sg.Web` — served by an ASP.NET Core server (`Sg.Server.exe`) that reuses the same `Sg.Domain` core. |
| DRS-side engineering frontend | **Browser webapp on WS2** served by `drs-server` — hardware health, per-variant monitor-scan, IP / network configuration, message logs. Used by the DRS Engineer. Framework: **React** (decided 2026-06-25, ADR-018). |
...
| Mode B Phase 1 (read-only browser viewer) | ASP.NET Core server + React SPA showing scenarios authored in WPF | ~1.5 months | **Deferred.** Activate when a customer requests browser delivery. |
| Mode B Phase 2 (full browser authoring) | React SPA implementing the full scenario authoring flow | ~3 months | **Deferred.** Customer-funded. |
...
- SG-side React developer (cross-trained to Python during ramp) — `drs-server` REST + RBAC + reports for v2 hardening; joins the React `Sg.Web` build when Mode B activates
- DRS-side React developer — DRS webapp on WS2: health dashboard, per-variant monitor-scan, IP / network configuration, message logs, per-variant control

When Mode B activates (post-acceptance, customer-funded), the SG-side frontend developer's primary work moves to the React `Sg.Web` SPA + a part-time GIS specialist (~6 weeks) is added for offline tile data preparation.
