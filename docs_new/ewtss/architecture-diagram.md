...
graph TB
    ...
    subgraph WS1["WS1 — Scenario Generator (SG)"]
        ...
        SGSRV["Sg.Server.exe + Sg.Web SPA<br/>ASP.NET Core + React + Cesium<br/><i>Mode B (deferred, opt-in)</i>"]
    ...
    subgraph WS2["WS2 — DRS (Device Replacement Software)"]
        direction TB
        DRSENG[/"👤 DRS Engineer"/]
        WEBAPP["<b>DRS Webapp</b><br/>browser on WS2<br/>served by drs-server<br/><b>React</b> (Vite + JS)<br/>per ADR-018"]
...
| **N3** | "Sg.Server.exe + Sg.Web SPA — ASP.NET Core + React + Cesium — *Mode B (deferred, opt-in)*" | WS1 subgraph | Standard internal — solid black border; italic label |
...
| **N6** | "**DRS Webapp** — browser on WS2 served by drs-server — React (Vite + JS) per ADR-018" | WS2 subgraph | Standard internal — solid black border |
