...
| Engineering team (today, v2 hardening phase) | Senior C# (×1), Senior Python (×1), Python (×2), C++ (×1), Cross-stack lead (×1), DRS-webapp frontend developer (React, ×1) |
| Engineering team (when Mode B activates, deferred) | + SG-side Frontend React/Cesium primary (E joins the React `Sg.Web` build; DRS-webapp G assists), + GIS specialist (~6 weeks part-time) |
...
- **Frontends — two by persona:**
  - **SG-side frontend** on WS1 for the Scenario Operator: scenario authoring, exercise control, reports, admin. Today: `Sg.App` WPF (Mode A). Future opt-in: `Sg.Web` React SPA (Mode B).
  - **DRS-side webapp** on WS2 for the DRS Engineer: hardware health, per-variant monitor-scan, IP/network configuration, message logs. Browser-based, served by `drs-server`. Framework decided at Milestone-1 design — **React** (per [ADR-018](decision-record.md#adr-018--ws2-drs-webapp-required-browser-frontend-on-the-drs-workstation-served-by-drs-server)).
...
**Future Mode B** (the browser-based React + CesiumJS SPA on WS1) would land in a third top-level directory (`sg-web/` or similar) when activated — not under `mvp4/`. It is deliberately not scaffolded today; see [ADR-001](decision-record.md) on the Hybrid architecture decision and [ADR-009](decision-record.md#adr-009--browser-spa-framework-mode-b-react--cesiumjs) on the framework.
...
│
├── drs-webapp/                         DRS Engineer SPA — React + Vite + JS (per ADR-018)
...
└── infrastructure/
    ├── kafka/
    │   ├── README.md                   native KRaft setup (no Docker) + topic inspection
...
┌─────────────── browser process ────────────────┐
│  Presentation layer                            │
│  React components + CesiumJS host              │
│  Sg.Web/                                       │
│            ▲                                   │
│            │ binds to                          │
│            ▼                                   │
...
- `InteractionController` knows about DTOs and `IScenarioBackend` but nothing about WPF, React, or the mouse event source. Mode A hands it `MouseDownEvent` from `WindowsFormsHost`; Mode B (future) will hand it pointer events from CesiumJS — same controller, two callers.
...
│        Mode B:  Sg.Web  (Electron shell + React app + CesiumJS viewer)       │
│      DRS-side (WS2, DRS Engineer):                                           │
│        DRS webapp (browser on WS2, served by drs-server; React + Vite)       │
...
- **L2 (Bridge) calls L3 (Kafka publish / consume) and L1 (TCP read / write) only.** Never calls L4 services synchronously over HTTP for high-frequency paths (the one exception: ResponseRouter in SCENARIO mode calls Sg.App's read-only scenario publisher endpoint per request — explicitly low-frequency, within the **≤30 ms p99 IRS budget** (measured ~10–17 ms uncached, <1 ms cached), see [Architecture Overview §4.5](#45-scenario-compute-and-link-analysis-write-back-scenario-modes-planning)).
...
- **View-models** — `AircraftViewModel`, `FacilityViewModel`, etc. — bind DTOs to the UI, dispatch user edits to `IScenarioBackend.UpdateXxx`. Used directly by Mode A; structure mirrored in Mode B's JavaScript view-models (JSDoc-typed against the OpenAPI contract).
...
### 3.4 `Sg.Web` (deferred) — Mode B React + CesiumJS SPA
...
- JavaScript view-models (JSDoc-typed) mirroring C# `Sg.Domain.ViewModels` shape.
- JavaScript `HttpScenarioBackend` mirroring `IScenarioBackend` interface (each method = one fetch call).
...
PostgreSQL 16 with the TimescaleDB 2.x extension, on WS2. **`drs-server` owns and migrates the entire schema (SQLAlchemy 2.0 + Alembic — not init scripts);** `Sg.App` is a direct-SQL write client (see [database-ownership-design](specs/database-ownership-design.md) and [Developer Handbook §10](developer-handbook.md#10-database-schema-reference) for the canonical schema). Three domains:

- **Scenario-planning** (written by `Sg.App`): `scenarios` (definition in `content_json`; `.sc`/`.vdf` are export-only, not retained between exports), `scenario_compute_snapshots`, `computed_links` *(hypertable — STK link budget, keyed on `scenario_id`)*, `reports`, `emitter_library`.
- **Execution + telemetry** (written by `drs-server`): `exercises` (the run record; `scenario_id` nullable — null = Random run), `drs_sessions`, `measurements` *(hypertable)* + `measurement_scalars`.
- **System** (written by `drs-server`): RBAC (`users`, `roles`, `user_roles`, `features`, `role_features` — multi-role per B1.16), instance `roster`, `system_logs` *(hypertable)*.

**Hypertables:** `computed_links`, `measurements`, `system_logs`. **Indexes:** publisher composite `(scenario_id, group_id, unit_id, tick_sec)` on `computed_links`; `(session_id, recorded_at, message_type)` on telemetry; B-tree on RBAC.
...
Browser-based SPA served by `drs-server` (§3.5) on WS2. Framework: **React in JavaScript** (Vite), decided 2026-06-25 per [ADR-018](decision-record.md#adr-018--ws2-drs-webapp-required-browser-frontend-on-the-drs-workstation-served-by-drs-server) and [v2 Execution Plan §3.7](v2-execution-plan.md#37-g-react--drs-webapp-lead) — the org's React teams standardise on JavaScript (which Angular forbids), plus an Angular-developer shortage; the Mode B `Sg.Web` SPA is React + JavaScript too ([ADR-009](decision-record.md#adr-009--browser-spa-framework-mode-b-react--cesiumjs)), so the v2 web frontend is a single React + JavaScript stack. Independent of the SG-side Mode A / Mode B selection — present in every deployment.
...
|---|---|---|---|---|
| **Standalone + Random** | **WS2 only** (DRS-Engineer-driven; WS1 not required — ADR-020) | None | C++ random generator | No |
| **Standalone + Scenario** | WS1 + WS2 | None | `computed_links` table (STK pre-computed) | Yes (WS1) |
| **Integrated + Random** | **WS2 + entity workstations** (WS1 not required — ADR-020) | Yes | C++ random generator | No |
...
- **Random** drives DRS values from DRS-Engineer-configured parameter ranges (no scenario, no STK). It is **WS2-only**: configured and run from the drs_webapp, so it does not require the SG Operator or the SG workstation to be up (ADR-020).
...
| `scenario.execution` | scenario publisher (Mode A: `Sg.App`; Mode B future: `Sg.Server`) | drs-bridge | Scenario modes only (per-tick data) |
| `drs.session.control` | `Sg.App` (Scenario) + drs-server (Random) | drs-bridge | run-control (start/stop), all modes |
| `drs.control` | drs-bridge | drs-server | all modes (variant.registered — **reverse direction**) |
| `drs.admin` | drs-server | drs-bridge | any mode — bridge-lifecycle admin (`restart_parser`/`restart_server`), diagnostic |

Always-on system/infra topics — `system.timesync`, `system.health`, `drs.roster` — are not mode-specific and are catalogued in [`contracts/README.md`](../../contracts/README.md).
...
- **What:** after compute completes, Sg.App keeps the computed_links result in-memory keyed by `(scenario_id, group_id, unit_id, tick)`. ResponseRouter requests are served from the cache instead of round-tripping to PostgreSQL.
...
Random replaces the scenario publisher with a per-variant random data generator inside the C++ parser. No `computed_links`, no STK, no scenario — and **no WS1**: Random mode is a DRS-Engineer, WS2-only capability (ADR-020). The SG Operator and SG workstation need not be running.

- The **DRS Engineer** configures parameter ranges (frequency min/max, power range, etc.) in the **drs_webapp** (WS2).
- The DRS Engineer starts a run → `drs-server` **writes the `exercises` run record** (mode=random, `scenario_id` null) and publishes a control message on `drs.session.control` → `drs-bridge` consumes it and begins generating bytes via `C++ generate_random(ranges)`.
- Bytes flow to Kafka `hw.<variant>.<kind>` and (in Integrated) also TCP-out to entity apps.
- The rest of the path (write DB, WS broadcast, frontend display on the drs_webapp) is identical to the Scenario modes.

This mode is for development, hardware checkout, and DRS-side diagnostics; the drs_webapp shows a health/status view rather than a scenario animation.
...
6. Frontend (WPF in Mode A; React in Mode B) updates dashboards in real time.
...
- **Frontend internal-layer decomposition** — the four-layer React / Cesium / Electron model (Electron shell → React app → feature modules → Cesium visualisation, with strict imports at each boundary) adapts the structure archived in [v2 archive §21.6](specs/v2-tech-stack-archive.md) (written against Angular, before the 2026-06-25 React decision). Becomes load-bearing for the frontend developer once hired.
