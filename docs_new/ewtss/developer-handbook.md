...
│
├── Sg.Web/                        [FUTURE — Mode B] React + CesiumJS
...
### 1.3 DRS webapp (React)

Owner: G ([v2 Execution Plan §3.7](v2-execution-plan.md#37-g-react--drs-webapp-lead)). Framework: React 18 + Vite + JavaScript (JSX), scaffolded per [ADR-018](decision-record.md) (decided 2026-06-25; re-scaffolded from the original Angular 18 slice). The webapp is a browser-served SPA hosted by `drs-server` on WS2 for the DRS Engineer persona.

```
drs-webapp/                              # DRS Engineer SPA
├── index.html                           # Vite entry
├── package.json                         # React 18, Vite, TanStack Query; Vitest + RTL tests
├── vite.config.js                       # React plugin, dev proxy, Vitest config
├── jsconfig.json                        # editor JSX support (no typecheck gate; checkJs off)
├── src/
│   ├── main.jsx                         # createRoot + QueryClientProvider + BrowserRouter
│   ├── styles.scss
│   ├── test-setup.js                    # jest-dom matchers for Vitest
│   ├── services/
│   │   ├── timeSync.js                  # getStatus() + useTimeSyncStatus() hook (5s poll); JSDoc typedefs
│   │   └── timeSync.test.js
│   └── app/
│       ├── App.jsx                      # `/dashboard` + redirect from `/`
│       ├── dashboard/
│       │   ├── Dashboard.jsx            # /dashboard route
│       │   └── time-sync-card/
│       │       ├── TimeSyncCard.jsx
│       │       └── TimeSyncCard.test.jsx
│       └── variants/
│           └── health/
│               └── TimeSyncRow.jsx      # per-variant row for Health detail
```
...
Cross-cutting: Git for Windows (Bash or PowerShell — repo supports both). Visual Studio 2022 (17.8+) or VS Code with C# Dev Kit + Python + ESLint extensions covers everything.
...
### 2.6 drs-webapp (React + Vite)

```
cd drs-webapp
npm install                                       # first time, ~20 s
npm test                                          # Vitest run (5 tests, ~2 s)
npm run build                                     # Vite production build into dist/drs-webapp/
npm run start                                     # Vite dev server on :4200
```

`npm run start` proxies `/time`, `/health`, and `/ws` to drs-server on `:8000` (see [vite.config.ts](../../drs-webapp/vite.config.ts)); for the dashboard to fetch `/time/status`, run drs-server locally. The card stays on `loading...` if drs-server is unreachable — that's the documented behaviour.

In production, the built `dist/drs-webapp/` is served statically by drs-server per [ADR-018](decision-record.md). The webapp framework is [ADR-018](decision-record.md) — **React in JavaScript** (Vite), decided 2026-06-25.
...
The full PostgreSQL 16 + TimescaleDB 2.x schema for EWTSS v2. The schema separates into three logical domains: **scenario planning** (the definition — written by the scenario publisher `Sg.App` today, `Sg.Server` future), **execution + time-series telemetry** (exercise runs + measurements — written by `drs-server`), and **system** (RBAC, logs, instance roster — written by `drs-server`).

> **Domain names = write ownership, not schema ownership.** All DDL for **all three** domains — scenario-planning included — is owned, created, and migrated by `drs-server` (single Alembic authority; see the migrations note below). `Sg.App` is a **cross-LAN write *client*** to the scenario-planning tables (`scenarios`, `computed_links`, …): it issues DML (`INSERT`/`UPDATE`/`DELETE`) directly against WS2 PostgreSQL but never DDL, and runs **no migrations of its own**. There is deliberately **no second migration system** (no EF Core / EF migrations, no C#-side schema tooling) anywhere in v2 — a single owner is what lets cross-domain foreign keys (e.g. `computed_links.scenario_id → scenarios`) stay DB-enforced and makes the DB-Purge cascade ([scenario-management-design §4.9](specs/scenario-management-design.md#49-db-purge-interaction)) safe. Full owner/writer/reader inventory and the SG-vs-DRS developer contract: [Database ownership model](specs/database-ownership-design.md).

> **Migrations vs init scripts (updated B1.43 Plan 2a):** the first DB-backed feature to ship is the **instance roster** ([roster store — Plan 2a](plans/drs-roster-store-plan.md)), which established `drs-server`'s DB foundation as **SQLAlchemy 2.0 async + Alembic** (migrations under `drs-server/alembic/`), not raw init SQL. New tables in **all three** domains — scenario-planning, telemetry, and system — follow that Alembic foundation; `drs-server` owns the migrations even for the scenario-planning tables that `Sg.App` writes at runtime (see the write-ownership note above). The earlier "`infrastructure/timescaledb/init/`" plan is superseded for `drs-server`-owned schema; any TimescaleDB-extension bootstrap (e.g. `CREATE EXTENSION timescaledb`) that must precede migrations is the remaining init-script candidate.

### 10.1 Scenario planning schema (scenario publisher writes)

```sql
-- NOTE (ADR-020): the scenario DEFINITION (entities, emitters, antennas, AOP,
-- environment, time window) lives inside scenarios.content_json (jsonb) — it is
-- NOT stored as normalized per-scenario child tables. The old exercises-rooted
-- child-table model (pre-B1.19) is superseded. A RUN of a scenario is an
-- `exercise` and lives in the telemetry/execution schema below (§10.2), written
-- by drs-server. See scenario-management-design §4.1 + contracts/scenario-write-surface.sql.

scenarios                            -- library of all scenarios (the DEFINITION)
  scenario_id (pk, uuid), name, owner
  content_json (jsonb)               -- { "compute_inputs": {entities, emitters, antennas,
                                     --     environment, time_window}, "metadata": {...} }
                                     -- inner shape: contracts/scenario-content-json-schema.json
  compute_inputs_hash (text)         -- sha256(content_json->'compute_inputs') — edit-invalidation
  compute_state (text)               -- NOT_COMPUTED | COMPUTED | STALE
  last_computed_at (timestamptz), last_computed_snapshot → scenario_compute_snapshots
  created_at, created_by, updated_at, updated_by, archived_at

scenario_compute_snapshots           -- immutable; one row per successful compute (report provenance)
  snapshot_id (pk, uuid), scenario_id → scenarios
  computed_at, computed_by, content_json (jsonb), compute_inputs_hash

computed_links                       -- STK link-analysis output (the RF link budget); TimescaleDB hypertable
  id, scenario_id → scenarios        -- keyed on the DEFINITION (computed once, replayed each run)
  computed_at (timestamptz)          -- partition key
  tick_sec (integer)                 -- seconds from scenario start (for scrubbing)
  group_id (integer), unit_id (integer) -- IRS Group/Unit ID (ICD §4), bound at compute; publisher hot-path key
  entity_id (uuid), emitter_id (uuid) -- = the stable per-instance `id`s from content_json (opaque, NOT FKs); display/provenance
  range_m, azimuth_deg, elevation_deg
  doppler_hz, signal_strength_dbm
  path_loss_db, link_margin_db
  is_visible (bool)
  -- REPLACE-on-recompute. Publisher index: (scenario_id, group_id, unit_id, tick_sec).
  -- Full DDL: contracts/scenario-write-surface.sql + scenario-management-design §4.1

reports                              -- interface sketch (full design is a separate B1.x item)
  report_id (pk, uuid), scenario_id → scenarios, snapshot_id → scenario_compute_snapshots
  generated_at, generated_by, pdf_path, baked_link_data (jsonb)

emitter_library                      -- reusable emitter templates (EW Library, B1.7 — cross-scenario)
  id, name, category, parameters_json (jsonb)
  created_by, is_system (bool)
```

### 10.2 Execution + time-series telemetry schema (`drs-server` writes)
...
```sql
exercises                            -- a RUN of a scenario, or a Random-mode run (ADR-020)
  id (pk, uuid), scenario_id → scenarios (NULLABLE — null = Random-mode run, no scenario)
  mode (text: scenario | random), scope (text: standalone | integrated)
  status (text: ready | executing | complete)
  created_by, created_at, started_at, ended_at
  -- Written by drs-server for BOTH modes. Scenario runs: Sg.App requests start via
  -- Kafka drs.session.control; drs-server mints the row. Random runs: DRS Engineer
  -- starts from the drs_webapp (WS2-only). Parent of drs_sessions.
...
users
  ...
  -- NB: no role_id — a user may hold MULTIPLE roles via user_roles (B1.16)

roles
  id, name (admin|sg_operator|drs_engineer)   -- B1.16 catalogue (no `observer`)
  -- Admin is LEAST-PRIVILEGE (user/system management + view, NOT authoring/run);
  -- operational authority comes from also holding sg_operator / drs_engineer.

user_roles                           -- many-to-many: a user's effective perms = union
  user_id → users, role_id → roles
  unique (user_id, role_id)

features                             -- capability keys granted to roles
  id, name, description
  -- catalogue (security-design §5): scenario.author, exercise.run.scenario,
  -- exercise.run.random, report.generate, telemetry.view, drs.config,
  -- drs.restart, log.view, user.manage, system.admin
  -- (drs.restart = restart TCP server/parser; named to avoid colliding with the
  --  Kafka `drs.control` topic — B1.16)
...
hardware_profiles                    -- variant TEMPLATE metadata
  id, hardware_type (varchar)
  protocol_version (varchar)
  config_json (jsonb)
  -- NOTE (B1.43): the original port / kafka_topic / parser_lib columns are
  -- SUPERSEDED. In v2 the per-variant TEMPLATE (parser_lib, time_signal, framing)
  -- lives in drs-bridge YAML profiles (Plan 1), and per-instance ports live in
  -- the roster (below). What remains here is non-addressing variant metadata.

-- INSTANCE ROSTER (B1.43 Plan 2a) — supersedes the old `ip_configurations`
-- sketch. Authoritative per-instance addressing; the RFQ "IP/network config
-- per variant" surface edits it; drs-bridge consumes the active roster.
-- See specs/drs-instance-addressing-design.md and plans/drs-roster-store-plan.md.
roster
  roster_id (pk, varchar), name, version (int), is_active (bool)
  updated_at, updated_by
  -- partial unique index: exactly one is_active = true

roster_entry
  id (pk), roster_id → roster
  instance_id (varchar)            -- logical id "<variant>#<n>"
  variant (varchar)
  host, command_port, command_protocol, response_port, response_protocol
  port_source (irs_fixed | allocated), enabled (bool)
  unique (roster_id, instance_id)

roster_revision                    -- append-only audit (one row per committed change)
  id (pk), roster_id, version, changed_at, changed_by, snapshot_json (jsonb)
...
|---|---|---|---|
| `computed_links` | `computed_at` (timestamptz) | Per scenario | Manual purge — scenarios are long-lived |
| `measurements` | `recorded_at` | 1 day | Configurable per deployment (default 90 days) |
| `system_logs` | `recorded_at` | 7 days | 90-day default |

### 10.5 Schema rules

- **`scenarios.content_json` is the single source of truth for a scenario definition** (entities, emitters, antennas, environment, time window — partitioned into `compute_inputs` vs `metadata`). Definitions are **not** normalized into per-scenario child tables (ADR-020). Don't shadow `content_json` fields in separate columns/tables.
- **Type-specific fields are generic (jsonb), never per-variant tables.** A new emitter/variant type with new RF parameters requires no schema migration — its fields live as keys inside `content_json.compute_inputs` and in `emitter_library.parameters_json`. (This is the surviving intent of the old `emitter_parameters` table, which is absorbed into `content_json`.)
...
- **`computed_links` is scenario-owned, REPLACE-on-recompute — not an exercise child.** It is keyed on `scenario_id` (ADR-020); recompute DELETEs then re-INSERTs the scenario's rows. Do **not** cascade-delete it from `exercises`.
- **Deleting a scenario is soft (archived_at) or an ordered purge, never a bare hard delete.** The `computed_links.scenario_id → scenarios` and `exercises.scenario_id → scenarios` FKs block a hard delete; DB Purge follows the ordered cascade in [scenario-management-design §4.9](specs/scenario-management-design.md#49-db-purge-interaction) (telemetry → `exercises` → `scenarios`). Deleting a `user` does NOT cascade — set `created_by` to a tombstone user.
...
class ResponseRouter:
    async def route(self, command: dict, profile: HardwareProfile) -> str:
        ...
        # Response SOURCE is chosen by MODE (random vs scenario). SCOPE
        # (standalone vs integrated) is ORTHOGONAL — Integrated additionally
        # relays to/from the entity controllers, regardless of mode. Do not
        # collapse mode and scope into one enum (a Random+Integrated run must
        # still hit the relay).
        if session is None or session.mode == Mode.RANDOM:
            response = self.random_gen.generate(command, profile)
        else:                                  # Mode.SCENARIO
            response = await self.scenario_client.get_scenario_response(
                session.exercise_id, command
            )
        if session and session.scope == Scope.INTEGRATED:
            await self.hardware_relay.forward(command, profile)
        return response
```

In the Hybrid:
- **Mode A (today):** the `scenario_client` makes HTTP calls to `Sg.App` running on WS1, which exposes a small read-only HTTP server during exercise execution. The endpoint is `GET /exercises/{id}/responses?group_id=X&unit_id=Y&tick=Z` — exercise-facing; `Sg.App` maps the active exercise to its `scenario_id` and returns the pre-computed response from `computed_links` (stored by `scenario_id` — ADR-020). The bridge never sees the scenario concept.
...
`drs-server` has no TCP servers and no STK. It is primarily a consumer — consumes Kafka, persists to TimescaleDB, serves REST and WebSocket queries to the SG-side operator UI on WS1, and serves the DRS webapp (static assets + REST/WS endpoints for health, monitor-scan, IP config, logs) to a local browser on WS2. It also owns **control-plane** paths (ADR-020): it writes the `exercises` run record via `POST /exercises` + `/exercises/{id}/stop`; for Random mode it **produces** `drs.session.control` start/stop (the run-control path the DRS Engineer drives from the webapp); and it **produces** `drs.admin` (`restart_parser`/`restart_server`) for the webapp's per-variant diagnostic restart, consumed by drs-bridge's AdminConsumer. So "pure consumer" holds for the telemetry ingest path, but drs-server is also the exercise-lifecycle writer and a control-plane producer.
...
A native single-node KRaft Kafka (no Docker — production is air-gapped Windows);
an idempotent Python script provisions the control-plane topics; the
...
- **Per-PR run** (every push to any feature branch or PR open):
  ...
  - DRS webapp build + unit tests (React + Vitest).
...
**Runtime-stack dependencies (DB + Kafka + JVM).** As of B1.43, `drs-server` depends on **PostgreSQL** (roster store; SQLAlchemy 2.0 async + asyncpg + Alembic — six allow-listed wheels, to be vendored under `packages/wheels/` at the delivery freeze per §15.8) and the system's telemetry bus is **Kafka**. The air-gapped delivery must ship these *runtimes*, not just our Python wheels — vendored as installers under `packages/installers/` (the Meinberg NTP MSI is the established pattern). All are recorded in [`packages/THIRD-PARTY-LICENCES.md`](../../packages/THIRD-PARTY-LICENCES.md). **One open licence item:** Kafka requires a JVM, and OpenJDK/Temurin is **GPLv2 + Classpath Exception** — the CPE is designed to keep the JVM's copyleft from reaching the apps that run on it, but the block-list above lists "GPL (all versions)", so the JVM choice needs **project-lead sign-off**. Dev-box install commands for all three (Postgres, native Kafka KRaft incl. the Windows `wmic` gotcha, JDK) are in the top-level [README "bring up local infra"](../../README.md).

### 15.8 Air-gap dependency vendoring process (B1.18)

The customer site is an **offline LAN**: nothing installs from PyPI / npm / the
internet. Every dependency — Python wheels, npm packages, C++ libraries, and the
*runtime* stack (PostgreSQL, Kafka, a JVM) — must be delivered on the DVD and
install offline. This section is the mechanical procedure; the licence gate is
§15.7, the catalogue is [`packages/THIRD-PARTY-LICENCES.md`](../../packages/THIRD-PARTY-LICENCES.md).

**Timing.** *Recording* a dependency (licence row + a `packages/` README) happens
**when it's adopted** — in the same PR. *Vendoring the binary* (committing the
wheel/installer) is a **pre-delivery freeze** activity against the final, pinned
dependency set — **not** done per-dep during early build, because the set still
churns and binaries bloat the repo. Until the freeze, `packages/` carries the
process + per-dependency READMEs with download instructions, **not** the binaries
(mirrors the Meinberg NTP installer: README now, MSI at the freeze).

**`.gitignore` contract (already in place):** `packages/**/*` is ignored *except*
directories, `*.md`, and `README*`. So vendored binaries are git-ignored by
default — you commit them deliberately at the freeze; the READMEs + licence index
always track.

**Directory layout under `packages/`:**

```
packages/
  wheels/<dist>/          Python wheels (the dep + its transitive deps)
  installers/<component>/ runtime installers/binaries (Postgres, Kafka, JDK, NTP)
  npm/<pkg>/              drs-webapp npm tarballs (when the webapp lands)
  cpp/<lib>/              third-party C++ source + built artefacts (.dll)
  THIRD-PARTY-LICENCES.md one row per vendored dependency (name, version, licence, source, SHA-256)
```

**Adding a Python dependency:**
1. Add it to the service `pyproject.toml`; confirm its licence is on the §15.7 allow-list.
2. Add a row to `packages/THIRD-PARTY-LICENCES.md` (name, version, licence, source URL; SHA-256 = *pending vendoring* until the freeze).
3. *(At the freeze)* download the wheel **plus its transitive deps**, pinned to the air-gap target's exact CPython ABI tag (Windows-x64; e.g. `cp311-cp311-win_amd64` — pin once WS2's Python is fixed):
   ```powershell
   pip download <dist> --dest packages\wheels\<dist>\ --only-binary=:all: `
       --python-version 311 --platform win_amd64 --implementation cp
   ```
   For **C-extension** wheels (`asyncpg`, `greenlet`, `markupsafe`) confirm a `*-cp311-cp311-win_amd64.whl` is present (not an sdist).
4. Verify offline install in a clean venv:
   ```powershell
   pip install --no-index --find-links=packages\wheels\<dist>\ <dist>
   python -c "import <module>; print('ok')"
   ```
5. Commit the wheels + the updated licence row together.

**Runtime installers (PostgreSQL, Kafka, JVM, NTP).** Large binaries behind
public sites — follow the Meinberg pattern: each gets
`packages/installers/<component>/README.md` with the download URL, a
`signtool verify /pa` (or checksum) step, and the SHA-256; the binary is copied
in at the freeze and pushed to the air-gap mirror. **The JVM choice needs §15.7
sign-off first** (OpenJDK is GPLv2+CPE).

**npm (drs-webapp) — build-from-source on the secure segment is required (2026-06-25, ADR-018):**
The webapp ships at runtime as a pre-built static bundle (WS1/WS2 run no Node), but
it must be *rebuildable* on the air-gapped LAN, so the build host carries the full
offline chain:
1. Confirm new deps are §15.7-allow-listed (`license-checker` covers prod **and** dev —
   the build needs devDependencies, e.g. Vite). Add rows to `THIRD-PARTY-LICENCES.md`.
2. Vendor the **Node.js 22 toolchain** under `packages/installers/nodejs/` (Node is a
   runtime, not an npm package; ⚠️ its bundled npm CLI is Artistic-2.0 → minor §15.7
   confirmation).
3. Populate the offline npm cache from the integrity-pinned lockfile **on a Windows-x64
   host** (so esbuild/Rollup platform binaries are captured): `npm ci --cache packages\npm\cache`.
4. Verify offline: `npm ci --offline --cache packages\npm\cache && npm run build` (the
   Vite build makes zero network requests). Full procedure: [`packages/npm/README.md`](../../packages/npm/README.md).

**C++ third-party libs:** vendor source + the built `.dll` under `packages/cpp/`,
pin the MSVC toolset version, record the licence.

**PR-time enforcement (planned CI gate, §15.7).** Any change to a
`pyproject.toml` / `requirements*.txt` / `package.json` / `CMakeLists.txt` must
touch `packages/THIRD-PARTY-LICENCES.md` in the same PR; CI fails otherwise.
Until hosted runners return, this is a manual review-checklist item.

**Currently pending vendoring (recorded; binaries deferred to the freeze):** the
six B1.43 roster-store wheels (SQLAlchemy, asyncpg, alembic, greenlet, Mako,
MarkupSafe) and the runtime stack (PostgreSQL, Kafka, JVM) — all in
`packages/THIRD-PARTY-LICENCES.md`.
