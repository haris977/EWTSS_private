...
**WS2-only Random mode.** Scenario-mode exercises require WS1 (STK compute + `Sg.App`). **Random-mode** exercises, however, are DRS-Engineer operations driven entirely from the drs_webapp on WS2 and are runnable **with WS1 powered off** ([ADR-020](decision-record.md#adr-020--scenario-is-the-definition-exercise-is-the-run-drs-server-owned-random-mode-is-ws2-only)). To support this, **WS2 carries its own local NTP time reference** (rather than depending on WS1 as the LAN Stratum-1 source) so a WS1-absent Random run is not auto-paused on `sync_lost`. See [random-mode-execution-design](specs/random-mode-execution-design.md).
...
WS2 is identical across SG-side Mode A and Mode B deployments. It hosts the entire telemetry pipeline (drs-bridge + Kafka + TimescaleDB + drs-server), the shared database, and the DRS webapp. The DRS webapp is served by drs-server to a local browser on WS2 itself; the DRS Engineer logs in to that browser for hardware health, per-variant monitor-scan, IP configuration, and message logs. Framework: **React in JavaScript** (Vite) per [ADR-018](decision-record.md#adr-018--ws2-drs-webapp-required-browser-frontend-on-the-drs-workstation-served-by-drs-server) + [v2 Execution Plan §3.7](v2-execution-plan.md#37-g-react--drs-webapp-lead), decided 2026-06-25. The built `dist/drs-webapp/` bundle is served statically by drs-server.
...
WS1 is the deployment's time source **when present**, per RFQ A.1 §F and [B1.3 design spec](specs/time-sync-design.md). The Meinberg NTP daemon runs on WS1 as a Stratum-1 server (LOCAL reference clock, stratum 10 fudge — air-gapped LAN has no upstream). WS2 (§5.6) and any other LAN consumer sync to it. For DRS-Engineer, WS2-only Random runs with WS1 powered off, WS2 falls back to its own local-clock reference (§1, §5.6; time-sync-design §4.5) rather than losing sync.
...
2. Loads `ws2-ntp.conf` template, substitutes `SG_HOST` with the value from the `-SgHost` parameter, writes the result to `C:\Program Files\NTP\etc\ntp.conf`. The config uses `iburst minpoll 4 maxpoll 6` on the SG peer for aggressive convergence, **plus a local-clock (`LOCAL`/orphan) reference at a high stratum** so WS2 keeps serving coherent time to its own services when WS1 is powered off — this is what makes DRS-Engineer, WS2-only Random runnable without WS1 (time-sync-design §4.5; §1 above).
3. Sets the NTP Windows Service to Automatic startup and starts it.
4. Waits 60 s for initial convergence.
5. Runs `ntpq -p` and asserts a synced system peer (`*`): the `<SgHost>` peer when WS1 is present, **or** the local-clock reference when WS1 is absent (a WS1-off WS2 still passes — it is disciplined to its own reference, not unsynced).
...
| Operation | Procedure |
|---|---|
| Daily startup (Scenario mode) | SG Operator launches `Sg.App.exe`. No manual telemetry restart required if WS2 services are configured as Windows Services with Automatic startup. |
| Daily startup (Random mode) | WS2-only — DRS Engineer opens the drs_webapp; no `Sg.App`/WS1 startup required ([ADR-020](decision-record.md#adr-020--scenario-is-the-definition-exercise-is-the-run-drs-server-owned-random-mode-is-ws2-only)). |
| Daily shutdown | Operator closes the WPF window; STK COM teardown completes within 5 s. Telemetry services remain running. |
| Backup | Scenarios live in TimescaleDB (`scenarios.content_json`) on WS2 — backed up with the DB via `pg_dump`, together with telemetry + link-analysis output, per site policy. (`.sc`/`.vdf` are operator export artefacts, not the source of truth, and are not retained between exports.) |
| Disaster recovery | Restore TimescaleDB from `pg_dump` — this restores scenarios, compute output, and telemetry together. STK reinstallation does not require licence migration if the licence is server-pulled. |
...
- **DRS webapp is built from source on the secure segment** (decision 2026-06-25; [ADR-018](decision-record.md#adr-018--ws2-drs-webapp-required-browser-frontend-on-the-drs-workstation-served-by-drs-server)). The build host on the air-gapped LAN needs the vendored **Node.js 22 toolchain** ([`packages/installers/nodejs/`](../../packages/installers/nodejs/README.md)) plus the **offline npm cache** ([`packages/npm/`](../../packages/npm/README.md)); `npm ci --offline && npm run build` runs with no internet (the Vite build makes zero network/CDN requests — verified). The resulting static `dist/drs-webapp/` bundle is what deploys to WS2 and is served by drs-server — **WS1/WS2 themselves run no Node/npm at runtime.** The npm cache must be populated on a Windows-x64 host so the platform-specific prebuilt binaries (esbuild/Rollup) are captured.
- **NPM packages bundled** [Mode B] — Mode B's `Sg.Web` React SPA follows the same build-from-source model on the secure segment; the built bundle is static assets served by `Sg.Server`.
