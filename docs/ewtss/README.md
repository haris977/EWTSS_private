...
| [ICD Open Questions](icd-open-questions.md) | C++ developer, integration lead, client meetings | 10 min (reference) | Unresolved questions from every reviewed ICD (JHF, JVU, DDF-550, DDF-1GTX, CA120, ESME, COMM DF, RSEC, AUS, RDFS). Categorised by risk — blockers, verify-before-live, nice-to-have. Use as the agenda for client hardware-integration meetings. |
...
- [HF parser per-group restructure — implementation plan](plans/hf-parser-restructure-plan.md) — 13-task plan splitting `dp_ecm_hf_parser.cpp` into 26 per-group files (13 parser + 13 format) under `src/hf/`. Task 1 creates scaffolding + shared headers + CMakeLists; Tasks 2–8 cover main HF groups (100, 101, 106, 109, 111, 112, 200); Tasks 9–12 cover MRx groups (1, 3, 4, 5, 6, 7); Task 13 trims the main file to ~150 lines.
